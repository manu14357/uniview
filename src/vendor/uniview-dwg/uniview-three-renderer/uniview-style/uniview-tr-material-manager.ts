import { UvGiSubEntityTraits, deepClone } from '@uniview/data-model'
 ...
 * - createMaterialImpl()
 */
export abstract class UvTrMaterialManager<T> {
  ...
  protected keyToTraits: Record<string, UvGiSubEntityTraits & T> = {}

  /** Options shared with subclasses (viewport scale, zoom uniforms, etc.) */
  protected options: UvTrStyleManagerOptions

  constructor(options: UvTrStyleManagerOptions) {
   ...
   */
  getMaterial(traits: UvGiSubEntityTraits, options: T): THREE.Material {
  ...
  protected abstract buildKey(traits: UvGiSubEntityTraits, options: T): string
