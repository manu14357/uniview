import { UvGeMatrix3d, UvGePoint3d, UvGiEntity } from '@uniview/data-model'
import * as THREE from 'three'

import { UvTrStyleManager } from '../uniview-style/uniview-tr-style-manager'
import { UvTrMaterialUtil, UvTrMatrixUtil } from '../uniview-util'
import { UvTrObject } from './uniview-tr-object'

/**
 * Represent the display object of one drawing entity.
 */
export class UvTrEntity extends UvTrObject implements UvGiEntity {
   ...
   */
  static flattenObject(root: UvTrEntity) {
   ...
   * @inheritdoc
   */
  applyMatrix(matrix: UvGeMatrix3d) {
    const threeMatrix = UvTrMatrixUtil.createMatrix4(matrix)
  ...
  protected highlightObject(object: THREE.Object3D) {
    if ('material' in object) {
      ...
      if (object.userData.originalMaterial == null) {
        const clonedMaterial = UvTrMaterialUtil.cloneMaterial(material)
   ...
   */
  fastDeepClone() {
    const cloned = new UvTrEntity(this.styleManager)
   ...
   * @inheritdoc
   */
  copy(object: UvTrEntity, recursive?: boolean) {
   ...
   */
  protected copyGeometry(source: UvTrEntity, target: UvTrEntity) {
