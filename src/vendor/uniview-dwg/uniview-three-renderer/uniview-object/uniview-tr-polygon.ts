...
export class UvTrPolygon extends UvTrEntity {
  constructor(
    area: UvGeArea2d,
  ...
  private buildHatchGeometry(
    ...
    const vec2Array = (vecs: UvGePoint2d[]) =>
  ...
  private findIntersectHole(
    pointBoundaries: UvGePoint2dLike[][],
    indexArr: number[]
  ) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      for (let j = i + 1; j < len; j++) {
        ...
        const isPolygonIntersect = UvGeGeometryUtil.isPolygonIntersect(
          points1,
          points2
        )
