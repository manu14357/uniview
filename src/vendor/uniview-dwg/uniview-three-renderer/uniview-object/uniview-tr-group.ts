import { UvDbObjectId, UvGeMatrix3d } from '@uniview/data-model'
import * as THREE from 'three'

import { UvTrStyleManager } from '../uniview-style/uniview-tr-style-manager'
import { UvTrMatrixUtil } from '../uniview-util'
import { UvTrEntity } from './uniview-tr-entity'
export interface UvTrEntityBox {
  minX: number
  minY: number
  maxX: number
  maxY: number
 ...
 * table and viewport.
 */
export class UvTrGroup extends UvTrEntity {
  private _isOnTheSameLayer: boolean
  private _boxes: UvTrEntityBox[] = []

  constructor(entities: UvTrEntity[], styleManager: UvTrStyleManager) {
    super(styleManager)
    entities.forEach(entity => {
      // FIXME: It looks like that code within 'Array.isArray(entity)' condition is useless.
      if (Array.isArray(entity)) {
        const subGroup = new UvTrEntity(styleManager)
        this.add(subGroup)
   ...
   * @inheritdoc
   */
  copy(object: UvTrGroup, recursive?: boolean) {
   ...
   */
  fastDeepClone() {
    const cloned = new UvTrGroup([], this.styleManager)
  ...
  private storeBoxes(object: THREE.Object3D) {
    if (object instanceof UvTrGroup) {
      object._boxes.forEach(box => this._boxes.push(box))
    } else if (object instanceof UvTrEntity) {
  ...
  private applyMatrixToEntityBox(box: UvTrEntityBox, matrix: THREE.Matrix4) {
    const points = [
