...
import {
  ...
  createUvTrBatchedMixin,
  createGeometryState,
  growCapacityIfNeeded,
  reserveGeometryId,
  resolveReservedCount
} from './uniview-tr-batched-mixin'

type UvTrBatchedLine2GeometryInfo = UvTrVertexBatchGeometryInfo
...
const UvTrBatchedLine2Base =
  createUvTrBatchedMixin<UvTrBatchedLine2GeometryInfo>(LineSegments2, {
    typeName: 'UvTrBatchedLine2',
 ...
 */
export class UvTrBatchedLine2 extends UvTrBatchedLine2Base {
  ...
  private _validateGeometry(geometry: LineSegmentsGeometry) {
    ...
    if (!start || !end) {
      throw new Error(
        'UvTrBatchedLine2: Geometry must have "instanceStart" and "instanceEnd" attributes.'
      )
    }
    if (
      start.itemSize !== 3 ||
      ...
      throw new Error(
        'UvTrBatchedLine2: Invalid line segment attributes. Expected matching vec3 instanceStart/instanceEnd.'
      )
    }
  }
  ...
  private _resizeSpaceIfNeeded(geometry: LineSegmentsGeometry) {
    ...
    const newMaxSegmentCount = growCapacityIfNeeded({
      ...
      growthFactor: UvTrBatchedLine2.GROWTH_FACTOR
    })
  ...
  addGeometry(
    ...
    const segmentCount = geometry.getAttribute('instanceStart').count
    const geometryInfo: UvTrBatchedLine2GeometryInfo = {
   ...
   */
  setGeometryInfo(geometryId: number, userData: UvTrBatchGeometryUserData) {
    if (geometryId >= this._geometryCount) {
      throw new Error('UvTrBatchedLine2: Maximum geometry count reached.')
    }
  ...
  setGeometryAt(geometryId: number, geometry: LineSegmentsGeometry) {
    ...
    if (segmentCount > geometryInfo.reservedVertexCount) {
      throw new Error(
        'UvTrBatchedLine2: Reserved space not large enough for provided geometry.'
      )
    }
