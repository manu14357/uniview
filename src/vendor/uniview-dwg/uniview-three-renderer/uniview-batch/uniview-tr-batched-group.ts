...
import { UvTrEntity } from '../uniview-object'
import { UvTrMaterialUtil } from '../uniview-util'
import { UvTrBatchGeometryUserData } from './uniview-tr-batched-geometry-info'
import { UvTrBatchedLine } from './uniview-tr-batched-line'
import { UvTrBatchedLine2 } from './uniview-tr-batched-line2'
import { UvTrBatchedMesh } from './uniview-tr-batched-mesh'
import { UvTrBatchedPoint } from './uniview-tr-batched-point'

export type UvTrBatchedObject =
  | UvTrBatchedLine
  | UvTrBatchedLine2
  | UvTrBatchedMesh

export interface UvTrEntityInBatchedObject {
...
export interface UvTrGeometrySize {
...
export interface UvTrGeometryInfo {
  /** Statistics for indexed geometry containers. */
  indexed: UvTrGeometrySize
  /** Statistics for non-indexed geometry containers. */
  nonIndexed: UvTrGeometrySize
}

export interface UvTrUnbatchedGroupStats {
...
export interface UvTrBatchedGroupStats {
  summary: {
  ...
  mesh: UvTrGeometryInfo
  /** Line batch statistics. */
  line: UvTrGeometryInfo
  /** Point batch statistics. */
  point: UvTrGeometryInfo
  /** Unbatched object statistics. */
  unbatched: UvTrUnbatchedGroupStats
}
 ...
 */
export class UvTrBatchedGroup extends THREE.Group {
   ...
   */
  private _entitiesMap: Map<string, UvTrEntityInBatchedObject[]>
  ...
  get stats() {
    ...
    const stats: UvTrBatchedGroupStats = {
      summary: {
  ...
  rerenderPoints(displayMode: number) {
    const creator = UvTrPointSymbolCreator.instance
   ...
   */
  addEntity(entity: UvTrEntity) {
    const objectId = entity.objectId
    const entityInfo: UvTrEntityInBatchedObject[] = []
  ...
  removeEntity(objectId: string) {
    ...
    if (entityInfo) {
      const batchedObjects = new Map<number, UvTrBatchedObject>()
  ...
  protected highlight(objectId: string, containerGroup: THREE.Group) {
    ...
    if (entityInfo && entityInfo.length < 1000) {
      entityInfo.forEach(item => {
        ...
        const clonedMaterial = UvTrMaterialUtil.cloneMaterial(object.material)
        UvTrMaterialUtil.setMaterialColor(clonedMaterial)
        object.material = clonedMaterial

        object.userData.objectId = objectId
        object.userData.disposeGeometryOnRemove =
          batchedObject instanceof UvTrBatchedLine2
    ...
    if (unbatchedObjects && unbatchedObjects.length < 1000) {
      unbatchedObjects.forEach(obj => {
        ...
        if (this.hasMaterial(highlightObj)) {
          const clonedMaterial = UvTrMaterialUtil.cloneMaterial(
  ...
  private addLine(
    ...
    userData: UvTrBatchGeometryUserData
  ): UvTrEntityInBatchedObject {
    const material = object.material as THREE.Material
    const batches = this.getMatchedLineBatches(object)
    let batchedLine = batches.get(material.id)
    if (batchedLine == null) {
      batchedLine = new UvTrBatchedLine(
        UvTrBatchedGroup.INITIAL_LINE_VERTEX_CAPACITY,
        UvTrBatchedGroup.INITIAL_LINE_INDEX_CAPACITY,
        material
      )
  ...
  private addLine2(
    ...
    if (batchedLine == null) {
      batchedLine = new UvTrBatchedLine2(
        UvTrBatchedGroup.INITIAL_LINE_VERTEX_CAPACITY,
        material
      )
  ...
  private addMesh(
    ...
    if (batchedMesh == null) {
      batchedMesh = new UvTrBatchedMesh(
        UvTrBatchedGroup.INITIAL_MESH_VERTEX_CAPACITY,
        UvTrBatchedGroup.INITIAL_MESH_INDEX_CAPACITY,
        material
      )
  ...
  private addPoint(
    ...
    if (batchedPoint == null) {
      batchedPoint = new UvTrBatchedPoint(
        UvTrBatchedGroup.INITIAL_POINT_VERTEX_CAPACITY,
        material
      )
  ...
  private getBatchedGeometrySize(
    batch:
      | Map<number, UvTrBatchedPoint>
      | Map<number, UvTrBatchedLine>
      | Map<number, UvTrBatchedLine2>
      | Map<number, UvTrBatchedMesh>
  ) {
    let memory = 0
    batch.forEach(value => {
   ...
   */
  private getUnbatchedStats(): UvTrUnbatchedGroupStats {
    const stats: UvTrUnbatchedGroupStats = {
      count: 0,
      geometrySize: 0,
      byType: {
        line: 0,
