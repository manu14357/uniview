...
import { UvTrCommonUtil } from '../uniview-util'
import {
  UvTrIndexedBatchGeometryInfo,
  UvTrVertexBatchGeometryInfo,
  ascIdSort,
  copyAttributeData
} from './uniview-tr-batched-geometry-info'

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type Constructor<T = object> = new (...args: any[]) => T

type UvTrBatchBaseObject = THREE.Object3D & {
...
type UvTrBatchGeometryLike = UvTrVertexBatchGeometryInfo
...
export function getMappingStats<T>(geometryInfo: T[]) {
  ...
  let size = 0
  if (count > 0) {
    size = UvTrCommonUtil.estimateObjectSize(geometryInfo[0])
  }
 ...
 */
export function deleteGeometryById<T extends UvTrBatchGeometryLike>(
  geometryId: number,
  geometryInfoList: T[],
  availableGeometryIds: number[]
) {
 ...
 */
export function validateGeometryId<T extends UvTrBatchGeometryLike>(
  geometryId: number,
  geometryInfoList: T[],
  typeName: string
) {
 ...
 */
export function applyGeometryAt<T extends UvTrBatchGeometryLike>(
  geometryInfo: T,
  batchGeometry: THREE.BufferGeometry,
  geometry: THREE.BufferGeometry,
  typeName: string
  ...
  if (
    (hasIndex &&
      srcIndex &&
      srcIndex.count >
        (geometryInfo as unknown as UvTrIndexedBatchGeometryInfo)
          .reservedIndexCount) ||
  ...
  if (hasIndex && srcIndex) {
    const indexedInfo = geometryInfo as unknown as UvTrIndexedBatchGeometryInfo
 ...
 */
export function computeBoundingBox<T extends UvTrBatchGeometryLike>(
 ...
 */
export function computeBoundingSphere<T extends UvTrBatchGeometryLike>(
...
export function initializeRaycastObject(
  raycastObject: UvTrBatchBaseObject,
  batchGeometry: THREE.BufferGeometry,
  material: THREE.Material | THREE.Material[]
) {
 ...
 */
export function resetRaycastObjectInfo(raycastObject: UvTrBatchBaseObject) {
 ...
 */
export function createUvTrBatchedMixin<
  TInfo extends UvTrBatchGeometryLike,
  TBase extends
    Constructor<UvTrBatchBaseObject> = Constructor<UvTrBatchBaseObject>
>(
  Base: TBase,
  options: {
    typeName: string
    createObject: () => UvTrBatchBaseObject
    getDrawRange: (
      instance: UvTrBatchBaseObject,
      info: TInfo
    ) => {
      start: number
      count: number
  ...
  /**
   * Base class produced by {@link createUvTrBatchedMixin}.
   */
  return class UvTrBatchedMixinBase extends Base {
    ...
    computeBoundingSphere() {
      this.boundingSphere = computeBoundingSphere(
        ...
        (geometryId, target) =>
          (
            this as unknown as UvTrBatchBaseObject & UvTrBatchBounds
    ...
    _initializeRaycastObject(raycastObject: UvTrBatchBaseObject) {
    ...
    _setRaycastObjectInfo(
      ...
      const self = this as unknown as UvTrBatchBaseObject & UvTrBatchBounds
    ...
    _resetRaycastObjectInfo(raycastObject: UvTrBatchBaseObject) {
