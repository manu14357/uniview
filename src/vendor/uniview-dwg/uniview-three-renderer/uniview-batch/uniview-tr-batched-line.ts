...
const UvTrBatchedLineBase = createUvTrBatchedMixin<UvTrBatchedGeometryInfo>(
  THREE.LineSegments,
  {
    typeName: 'UvTrBatchedLine',
 ...
 */
export class UvTrBatchedLine extends UvTrBatchedLineBase {
  ...
  private _validateGeometry(geometry: THREE.BufferGeometry) {
    validateGeometry(this.geometry, geometry, 'UvTrBatchedLine', true)
  }
    ...
    const newMaxIndexCount =
      index == null
        ...
        : growCapacityIfNeeded({
            ...
            growthFactor: UvTrBatchedLine.GROWTH_FACTOR
          })
   ...
   */
  getUserData() {
    const userData: UvTrBatchGeometryUserData[] = []
  ...
  addGeometry(
    ...
    const index = geometry.getIndex()
    const geometryInfo: UvTrBatchedGeometryInfo = {
  ...
  setGeometryInfo(geometryId: number, userData: UvTrBatchGeometryUserData) {
    if (geometryId >= this._geometryCount) {
      throw new Error('UvTrBatchedLine: Maximum geometry count reached.')
    }
  ...
  setGeometryAt(geometryId: number, geometry: THREE.BufferGeometry) {
    ...
    const geometryInfo = this._geometryInfo[geometryId]
    applyGeometryAt(geometryInfo, batchGeometry, geometry, 'UvTrBatchedLine')
