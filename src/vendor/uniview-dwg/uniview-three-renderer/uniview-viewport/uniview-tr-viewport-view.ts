import { UvGeBox2d, UvGiViewport } from '@uniview/data-model'
import * as THREE from 'three'

import { UvTrRenderer } from '../uniview-renderer'
import { UvTrBaseView } from './uniview-tr-base-view'

/**
 * This class represents view to show viewport.
 */
export class UvTrViewportView extends UvTrBaseView {
  private _parentView: UvTrBaseView
  private _viewport: UvGiViewport

  /**
   * Calcuate the bounding box of this viewport in client window coordinate system
   */
  static calculateViewportWindowBox(
    parentView: UvTrBaseView,
    viewport: UvGiViewport
  ) {
    const wcsViewportBox = viewport.box
    const cwcsViewportBox = new UvGeBox2d()
  ...
  constructor(
    ...
    const viewportWindowBox = UvTrViewportView.calculateViewportWindowBox(
      parentView,
      viewport
    )
