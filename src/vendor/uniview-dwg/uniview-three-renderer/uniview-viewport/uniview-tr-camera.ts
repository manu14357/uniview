import { UvGePoint2d, UvGePoint2dLike } from '@uniview/data-model'
import * as THREE from 'three'

export class UvTrCamera {
   ...
   */
  screenToWorld(point: UvGePoint2dLike, width: number, height: number) {
    ...
    return new UvGePoint2d(wcsPos.x, wcsPos.y)
  }
  /**
   ...
   */
  worldToScreen(point: UvGePoint2dLike, width: number, height: number) {
   ...
   */
  public wcs2Ndc(point: UvGePoint2dLike, width: number, height: number) {
   ...
   * Bottom-left cooridinate in NDC (Normalized screen coordinate) is (-1, -1) and top-right is (1, 1).
   */
  public cwcs2Ndc(point: UvGePoint2dLike, width: number, height: number) {
    return new UvGePoint2d(
      (point.x / width) * 2 - 1,
      -(point.y / height) * 2 + 1
    )
