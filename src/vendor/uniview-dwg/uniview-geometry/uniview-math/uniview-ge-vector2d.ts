...
import { UvGeMathUtil } from '../uniview-util/uniview-ge-math-util'
import { UvGeMatrix2d } from './uniview-ge-matrix2d'
import { UvGePoint2dLike } from './uniview-ge-point2d'

/**
 * The interface representing a 2d vector.
 */
export interface UvGeVector2dLike {
  /**
 ...
 */
export class UvGeVector2d {
  static EMPTY = Object.freeze(new UvGeVector2d(0, 0))
  ...
  constructor(other: UvGeVector2dLike)
  ...
  constructor(a?: unknown, b?: unknown) {
    ...
    if (argsLength === 1) {
      const { x, y } = a as UvGeVector2dLike
      this.x = x
      this.y = y
      return
    }
   ...
   */
  clone() {
    return new UvGeVector2d(this.x, this.y)
  }
   ...
   * @returns Return this vector
   */
  addVectors(a: UvGeVector2d, b: UvGeVector2d) {
    this.x = a.x + b.x
    this.y = a.y + b.y
   ...
   * @returns Return this vector
   */
  addScaledVector(v: UvGeVector2d, s: number) {
    this.x += v.x * s
    this.y += v.y * s
   ...
   * @returns Return this vector
   */
  subVectors(a: UvGeVector2dLike, b: UvGeVector2dLike) {
    this.x = a.x - b.x
    this.y = a.y - b.y
    return this
  }
   ...
   * @returns
   */
  clamp(min: UvGeVector2dLike, max: UvGeVector2dLike) {
   ...
   */
  distanceToSquared(v: UvGeVector2dLike) {
    const dx = this.x - v.x,
      dy = this.y - v.y
    return dx * dx + dy * dy
  }
   ...
   */
  manhattanDistanceTo(v: UvGeVector2d) {
   ...
   * @returns Return this vector
   */
  lerp(v: UvGeVector2d, alpha: number) {
   ...
   * @returns
   */
  lerpVectors(v1: UvGeVector2d, v2: UvGeVector2d, alpha: number) {
   ...
   * @returns Return this vector
   */
  rotateAround(center: UvGePoint2dLike, angle: number) {
  ...
  relativeEps(epsilon = 1.0e-7) {
    return Math.min(
      UvGeMathUtil.relativeEps(this.x, epsilon),
      UvGeMathUtil.relativeEps(this.y, epsilon)
    )
  }
