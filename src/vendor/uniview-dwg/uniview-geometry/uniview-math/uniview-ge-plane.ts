import { UvGeBox3d } from './uniview-ge-box3d'
...
const _vector1 = /*@__PURE__*/ new UvGeVector3d()
const _vector2 = /*@__PURE__*/ new UvGeVector3d()
const _normalMatrix = /*@__PURE__*/ new UvGeMatrix2d()

/**
 * A two dimensional surface that extends infinitely in 3d space, represented in Hessian normal form
 * by a unit length normal vector and a constant.
 */
export class UvGePlane {
  /**
   * The unit-length normal vector of the plane.
   * **Important**: This vector is assumed to be normalized. Use `normalize()` after
   * modifying it to maintain the invariant.
   */
  normal: UvGeVector3d
  /**
   ...
   */
  constructor(normal = new UvGeVector3d(1, 0, 0), constant = 0) {
   ...
   */
  set(normal: UvGeVector3dLike, constant: number) {
   ...
   * @returns Return this plane
   */
  setFromNormalAndCoplanarPoint(normal: UvGeVector3dLike, point: UvGeVector3d) {
    this.normal.copy(normal)
   ...
   * @returns Return this plane
   */
  setFromCoplanarPoints(a: UvGeVector3d, b: UvGeVector3d, c: UvGeVector3d) {
   ...
   */
  projectPoint(point: UvGeVector3dLike, target: UvGeVector3d) {
    return target
      .copy(point)
   ...
   * @returns Return the target
   */
  // intersectLine(line: Line3, target: UvGeVector3d) {
   ...
   */
  coplanarPoint(target: UvGeVector3d) {
   ...
   * @returns Return this plane
   */
  applyMatrix4(matrix: UvGeMatrix3d, optionalNormalMatrix: UvGeMatrix2d) {
    const normalMatrix =
   ...
   */
  clone() {
    return new UvGePlane().copy(this)
  }
}
