 ...
 */
export class UvGeCircArc2d extends UvGeCurve2d {
  ...
  constructor(p1: UvGePoint2dLike, p2: UvGePoint2dLike, p3: UvGePoint2dLike)
  constructor(start: UvGePoint2dLike, end: UvGePoint2dLike, bulge: number)
  constructor(
    center: UvGePoint2dLike,
    radius: number,
    startAngle: number,
    endAngle: number,
    clockwise: boolean
    ...
    if (argsLength == 3) {
      if (
        ...
        this.createByThreePoints(
          a as UvGePoint2dLike,
          b as UvGePoint2dLike,
          c as UvGePoint2dLike
        )
      } else {
      ...
      const center = a as UvGePoint2dLike
      this.center = new UvGePoint2d(center.x, center.y)
      this.radius = b as number
      this._clockwise = e as boolean
      // Store internal angles (unmirrored)
      this._startAngle = this._clockwise
        ? this._mirrorAngle(UvGeMathUtil.normalizeAngle(c as number))
        : UvGeMathUtil.normalizeAngle(c as number)
      this._endAngle = this._clockwise
        ? this._mirrorAngle(UvGeMathUtil.normalizeAngle(d as number))
        : UvGeMathUtil.normalizeAngle(d as number)
    } else {
  ...
  private createByThreePoints(
    ...
    const slope = (p1: UvGePoint2dLike, p2: UvGePoint2dLike): number =>
      (p2.y - p1.y) / (p2.x - p1.x)
    ...
    const angle = (p: UvGePoint2dLike, center: UvGePoint2dLike): number =>
  ...
  private createByStartEndPointsAndBulge(
    ...
    if (bulge < 0) {
      ...
      a = new UvGeVector2d(from)
      b = new UvGeVector2d(to)
    } else {
      // Default is counter-clockwise
      theta = Math.atan(bulge) * 4
      a = new UvGeVector2d(to)
      b = new UvGeVector2d(from)
    }

    const ab = new UvGeVector2d().subVectors(b, a)
    const lengthAB = ab.length()
    const c = new UvGeVector2d().addVectors(a, ab.multiplyScalar(0.5))
   ...
   */
  get center(): UvGePoint2d {
    return this._center
  }
  set center(value: UvGePoint2dLike) {
    this._center = new UvGePoint2d(value.x, value.y)
  ...
  set startAngle(value: number) {
    this._startAngle = this._clockwise
      ? this._mirrorAngle(UvGeMathUtil.normalizeAngle(value))
  ...
  get deltaAngle() {
    ...
    return this.clockwise
      ? UvGeMathUtil.normalizeAngle(internalStartAngle - internalEndAngle)
      : UvGeMathUtil.normalizeAngle(internalEndAngle - internalStartAngle)
  }
   ...
   */
  get midPoint(): UvGePoint2d {
    const internalStartAngle = this._getInternalAngle(this.startAngle)
    const internalEndAngle = this._getInternalAngle(this.endAngle)
    const internalMidAngle = UvGeMathUtil.normalizeAngle(
  ...
  calculateBoundingBox(): UvGeBox2d {
    ...
    return new UvGeBox2d(
      new UvGePoint2d(Math.min(...xValues), Math.min(...yValues)),
      new UvGePoint2d(Math.max(...xValues), Math.max(...yValues))
    )
  }
   ...
   */
  getPointAtAngle(angle: number): UvGePoint2d {
    const internalAngle = this._getInternalAngle(angle)
   ...
   */
  getPoints(numPoints: number = 100): UvGePoint2d[] {
