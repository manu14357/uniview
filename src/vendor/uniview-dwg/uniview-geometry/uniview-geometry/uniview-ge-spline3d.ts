...
import { UvGeKnotParameterizationType, UvGeNurbsCurve } from './uniview-ge-nurbs-curve'

export class UvGeSpline3d extends UvGeCurve3d {
  private _nurbsCurve: verb.geom.NurbsCurve
  private _fitPoints?: UvGePoint3dLike[]
  private _knotParameterization?: UvGeKnotParameterizationType
  ...
  constructor(a?: unknown, b?: unknown, c?: unknown, d?: unknown, e?: unknown) {
    ...
    if (!Array.isArray(b)) {
      ...
      this._fitPoints = a as UvGePoint3dLike[]
      this._knotParameterization = b as UvGeKnotParameterizationType
      ...
      this._controlPoints = a as UvGePoint3dLike[]
  ...
  private buildCurve() {
    if (this._fitPoints && this._knotParameterization) {
      ...
      if (this._closed) {
        const newFitPoints = UvGeNurbsCurve.createFitPointsForClosedCurve(
          this._fitPoints
        )
  ...
  get startPoint(): UvGePoint3d {
    ...
    return new UvGePoint3d(startPoint[0], startPoint[1], startPoint[2])
  }
  ...
  get endPoint(): UvGePoint3d {
    ...
    return new UvGePoint3d(endPoint[0], endPoint[1], endPoint[2])
  }
   ...
   * @returns
   */
  getFitPointAt(index: number): UvGePoint3dLike {
    if (!this._fitPoints) {
  ...
  getPoints(numPoints: number = 100): UvGePoint3d[] {
    ...
    for (let i = 0; i < numPoints; i++) {
      ...
      points.push(new UvGePoint3d(point[0], point[1], point[2]))
    }
    return points
  }

  getCurvePoints(curve: UvGeNurbsCurve, count: number) {
    const points = []
  ...
  calculateBoundingBox() {
    ...
    return new UvGeBox3d().setFromPoints(points)
  }
   ...
   */
  private toNurbsPoints(points: UvGePoint3dLike[]): number[][] {
   ...
   * @returns Return converted points
   */
  private toVerbPoints(points: UvGePointLike[]): number[][] {
   ...
   * @returns Return converted points
   */
  private toGePoints(points: number[][]): UvGePoint3dLike[] {
    const gePoints = new Array<UvGePoint3dLike>(points.length)
    points.forEach((point, index) => {
  ...
  /**
   * Create a closed spline from fit points using UvGeNurbsCurve.createClosedCurve
  ...
  static createClosedSpline(
    ...
    return new UvGeSpline3d(fitPoints, parameterization, degree, true)
  }
}
