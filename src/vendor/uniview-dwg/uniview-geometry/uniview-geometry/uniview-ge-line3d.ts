...
import { UvGeMathUtil } from '../uniview-util'
 ...
 */
export class UvGeLine3d extends UvGeCurve3d {
  private _start: UvGePoint3d
  private _end: UvGePoint3d
  /**
   ...
   */
  constructor(start: UvGePoint3dLike, end: UvGePoint3dLike) {
    super()
    this._start = new UvGePoint3d(start)
    this._end = new UvGePoint3d(end)
  }
  ...
  set startPoint(value: UvGePoint3dLike) {
  ...
  set endPoint(value: UvGePoint3dLike) {
   ...
   */
  isPointOnLine(point: UvGePoint3dLike): boolean {
   ...
   */
  at(t: number, target: UvGePoint3d) {
  ...
  atLength(length: number, flag: boolean = false) {
    if (flag) {
      ...
      return new UvGePoint3d(this._start).addScaledVector(direction, length)
    } else {
      const direction = this.delta(_vector).normalize()
      return new UvGePoint3d(this._end).addScaledVector(direction, length)
    }
  }
  ...
  extend(length: number, inversed: boolean = false) {
    if (inversed) {
      ...
      this._start = new UvGePoint3d(this._start).addScaledVector(
        direction,
        length
      )
    } else {
      const direction = this.delta(_vector).normalize()
      this._end = new UvGePoint3d(this._end).addScaledVector(direction, length)
    }
    this._boundingBoxNeedsUpdate = true
    return this
  }
   ...
   */
  closestPointToPointParameter(point: UvGePoint3d, clampToLine: boolean) {
   ...
   */
  perpPoint(point: UvGePoint3dLike) {
    const lineDirection = this.direction
   ...
   */
  clone() {
    return new UvGeLine3d(this._start.clone(), this._end.clone())
  }
}

const _vector = /*@__PURE__*/ new UvGeVector3d()
const _startP = /*@__PURE__*/ new UvGeVector3d()
const _startEnd = /*@__PURE__*/ new UvGeVector3d()
