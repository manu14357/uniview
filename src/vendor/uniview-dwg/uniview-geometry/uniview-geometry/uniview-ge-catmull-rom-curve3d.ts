 ...
 * const curve = new UvGeCatmullRomCurve3d( [
 *   new UvGePoint3d( -10, 0, 10 ),
 *   new UvGePoint3d( -5, 5, 5 ),
 ...
 * ```
 */
export class UvGeCatmullRomCurve3d extends UvGeCurve3d {
  /**
  ...
  private readonly _tmp = new UvGeVector3d()
  ...
  constructor(
    ...
    super()
    this._points = points.map(p => new UvGePoint3d(p))
    this._closed = closed
  ...
  get startPoint(): UvGePoint3d {
    return this._points.length > 0 ? this._points[0] : new UvGePoint3d()
  }
   ...
   */
  getPoint(t: number, optionalTarget = new UvGePoint3d()): UvGePoint3d {
    ...
    let p0: UvGePoint3d, p3: UvGePoint3d // 4 points (p1 & p2 defined below)
    ...
    if (this._closed || intPoint > 0) {
      ...
      p0 = new UvGePoint3d(this._tmp.x, this._tmp.y, this._tmp.z)
    }
    ...
    if (this._closed || intPoint + 2 < l) {
      ...
      p3 = new UvGePoint3d(this._tmp.x, this._tmp.y, this._tmp.z)
    }
   ...
   */
  getPoints(divisions: number): UvGePoint3d[] {
   ...
   */
  setPoints(points: UvGePoint3dLike[]): void {
    this._points = points.map(p => new UvGePoint3d(p))
  ...
  transform(matrix: UvGeMatrix3d): this {
    this._points = this._points.map(point => {
      const transformedPoint = new UvGePoint3d()
   ...
   */
  protected calculateBoundingBox(): UvGeBox3d {
