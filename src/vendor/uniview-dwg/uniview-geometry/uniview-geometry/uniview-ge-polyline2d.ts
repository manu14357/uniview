 ...
 */
export interface UvGePolyline2dVertex {
  x: number
  y: number
  /**
 ...
 */
export class UvGePolyline2d<
  T extends UvGePolyline2dVertex = UvGePolyline2dVertex
> extends UvGeCurve2d {
   ...
   *
   * Notes:
   * This property returns a read-only view of the internal vertices array.
   * Use `addVertexAt()` or `removeVertexAt()` to modify vertices.
   */
  get vertices(): ReadonlyArray<T> {
  ...
  get startPoint(): UvGePoint2d {
    if (this.numberOfVertices > 0) {
      ...
      return new UvGePoint2d(vertex.x, vertex.y)
    }
  ...
  get length() {
    ...
    for (let index = 0; index < vertexArraylength; ++index) {
      ...
      let nextVertex: UvGePolyline2dVertex | null = null
      ...
      if (nextVertex) {
        if (vertex.bulge) {
          const arc = new UvGeCircArc2d(vertex, nextVertex, vertex.bulge)
          length += arc.length
        } else {
          length += new UvGePoint2d(vertex.x, vertex.y).distanceTo(nextVertex)
        }
      }
    }
    return length
   ...
   */
  getPointAt(index: number): UvGePoint2d {
    const vertex = this._vertices[index]
    return new UvGePoint2d(vertex.x, vertex.y)
  }
  ...
  calculateBoundingBox(): UvGeBox2d {
    ...
    return new UvGeBox2d().setFromPoints(points)
  }
  ...
  getPoints3d(numPoints: number, elevation: number) {
    ...
    tmp.forEach(point =>
      points.push(new UvGePoint3d().set(point.x, point.y, elevation))
    )
    return points
  }
  ...
  getPoints(numPoints: number): UvGePoint2d[] {
    ...
    for (let index = 0; index < length; ++index) {
      ...
      if (vertex.bulge) {
        ...
        points.push(new UvGePoint2d(vertex.x, vertex.y))
