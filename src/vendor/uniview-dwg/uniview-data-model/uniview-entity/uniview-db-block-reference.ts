...
import { UvGiEntity, UvGiRenderer } from '@uniview/graphics'

import { UvDbObjectId } from '../uniview-base'
import { UvDbObjectIterator, UvDbOsnapMode, UvDbRenderingCache } from '../uniview-misc'
import { UvDbAttribute } from './uniview-db-attribute'
 ...
 * const blockRef = new UvDbBlockReference("MyBlock");
 * blockRef.position = new UvGePoint3d(10, 20, 0);
 * blockRef.rotation = Math.PI / 4; // 45 degrees
 * blockRef.scaleFactors = new UvGePoint3d(2, 2, 1); // 2x scale
 *
 ...
 * ```
 */
export class UvDbBlockReference extends UvDbEntity {
  ...
  private _scaleFactors: UvGePoint3d
  ...
  private _attribs: Map<string, UvDbAttribute>
   ...
   * ```typescript
   * const blockRef = new UvDbBlockReference("MyBlock");
   * blockRef.position = new UvGePoint3d(5, 10, 0);
  ...
  constructor(blockName: string) {
    ...
    this._scaleFactors = new UvGePoint3d(1, 1, 1)
   ...
   * @example
   * ```typescript
   * blockRef.position = new UvGePoint3d(15, 25, 0);
   * ```
   */
   ...
   * @example
   * ```typescript
   * blockRef.scaleFactors = new UvGePoint3d(2, 1.5, 1); // 2x X scale, 1.5x Y scale
   * ```
   */
  set scaleFactors(value: UvGePoint3dLike) {
   ...
   * @example
   * ```typescript
   * blockRef.normal = new UvGeVector3d(0, 0, 1);
   * ```
   */
   ...
   */
  attributeIterator(): UvDbObjectIterator<UvDbAttribute> {
    return new UvDbObjectIterator(this._attribs)
  }
   ...
   * (see {@link UvDbRenderingCache.draw}), matching AutoCAD / RealDWG behavior.
   *
  ...
  get blockTransform(): UvGeMatrix3d {
    ...
    // ------------------------------------------------------------
    const mScale = new UvGeMatrix3d().makeScale(
    ...
    // ------------------------------------------------------------
    const qRot = new UvGeQuaternion().setFromAxisAngle(
      UvGeVector3d.Z_AXIS,
      this._rotation
    )
    const mRot = new UvGeMatrix3d().makeRotationFromQuaternion(qRot)
   ...
   *
   * The geometry group exposes editable properties via {@link UvDbPropertyAccessor}
  ...
  get properties(): UvDbEntityProperties {
    const props: UvDbEntityProperties = {
      type: this.type,
      groups: [
    ...
    if (this._attribs.size > 0) {
      const group: UvDbEntityPropertyGroup = {
        groupName: 'attribute',
        properties: []
      }
  ...
  subWorldDraw(renderer: UvGiRenderer) {
    ...
    if (blockTableRecord != null) {
      ...
      const block = UvDbRenderingCache.instance.draw(
        renderer,
        blockTableRecord,
        this.rgbColor,
        attribs,
