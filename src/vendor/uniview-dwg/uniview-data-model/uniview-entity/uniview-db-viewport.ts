import { UvGeBox3d, UvGePoint3d } from '@uniview/geometry'
import {
  UvGiEntity,
  UvGiRenderer,
  UvGiViewport
 ...
 * @example
 * ```typescript
 * const viewport = new UvDbViewport();
 * viewport.centerPoint = new UvGePoint3d(0, 0, 0);
 * viewport.width = 10;
 * viewport.height = 8;
 * viewport.number = 2;
 * ```
 */
export class UvDbViewport extends UvDbEntity {
  ...
  /**
   * Creates a new UvDbViewport instance.
   *
  ...
  /**
   * Converts this UvDbViewport to an UvGiViewport for rendering purposes.
   *
   ...
   *
   * @returns An UvGiViewport instance with all viewport properties copied
   */
  toGiViewport() {
   ...
   * @private
   */
  private createViewportRect(viewport: UvGiViewport, renderer: UvGiRenderer) {
    const lines: UvGiEntity[] = []
    lines.push(
      renderer.lines([
        new UvGePoint3d(
