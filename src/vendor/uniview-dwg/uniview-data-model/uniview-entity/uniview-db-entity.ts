...
import { UvDbObject } from '../uniview-base/uniview-db-object'
import { UvDbOsnapMode, ByBlock, ByLayer, DEFAULT_LINE_TYPE } from '../uniview-misc'
import {
  UvDbEntityProperties,
  UvDbEntityPropertyGroup
} from './uniview-db-entity-properties'
...
export abstract class UvDbEntity extends UvDbObject {
  ...
  private _transparency: UvCmTransparency = new UvCmTransparency()
   ...
   * @example
   * ```typescript
   * const entity = new UvDbLine();
   * console.log(entity.type); // "Line"
   * ```
   */
  get type() {
    return (this.constructor as typeof UvDbEntity).typeName
  }
   ...
   * @example
   * ```typescript
   * entity.color = new UvCmColor(0xFF0000);
   * ```
   */
  set color(value: UvCmColor) {
    if (this._color == null) this._color = new UvCmColor()
  ...
  get lineWeight() {
    if (this._lineWeight == null) {
      this._lineWeight = this.database.celweight ?? UvGiLineWeight.ByLayer
    }
   ...
   */
  set lineWeight(value: UvGiLineWeight) {
   ...
   * ```
   */
  set transparency(value: UvCmTransparency) {
   ...
   *
   * Note: The `groups` array contains `UvDbEntityPropertyGroup` objects whose
   * `properties` entries are runtime-resolved property descriptors that include
   * {@link UvDbPropertyAccessor} objects. Each property object therefore
   * conforms to `UvDbEntityRuntimeProperty`.
   */
   ...
   * entity only. In AutoCAD, it uses UvGiSubEntityTraits::setSelectionMarkerInput
   ...
   * @example
   * ```typescript
   * const snapPoints: UvGePoint3d[] = [];
   * entity.subGetOsnapPoints(UvDbOsnapMode.Endpoint, 0, pickPoint, lastPoint, snapPoints);
   * ```
   */
  subGetOsnapPoints(
    ...
    // @ts-expect-error not use '_' prefix so that typedoc can the correct parameter to generate doc
    pickPoint: UvGePoint3dLike,
    // @ts-expect-error not use '_' prefix so that typedoc can the correct parameter to generate doc
    lastPoint: UvGePoint3dLike,
    // @ts-expect-error not use '_' prefix so that typedoc can the correct parameter to generate doc
   ...
   * @example
   * ```typescript
   * const matrix = UvGeMatrix3d.translation(10, 0, 0);
   * entity.transformBy(matrix);
   * ```
   */
  // @ts-expect-error not use '_' prefix so that typedoc can the correct parameter to generate doc
  transformBy(matrix: UvGeMatrix3d): this {
    return this
  }
   ...
   * ```
   */
  abstract get geometricExtents(): UvGeBox3d
  ...
  abstract subWorldDraw(
    renderer: UvGiRenderer,
    delay?: boolean
  ): UvGiEntity | undefined
   ...
   * @returns The rendered entity, or undefined if drawing failed
   */
  worldDraw(renderer: UvGiRenderer, delay?: boolean): UvGiEntity | undefined {
   ...
   * (e.g., handle, layer). Each property includes a runtime {@link UvDbPropertyAccessor}
   ...
   */
  protected getGeneralProperties(): UvDbEntityPropertyGroup {
    return {
      groupName: 'general',
      ...
      properties: [
        ...
        },
        {
          ...
          editable: true,
          accessor: {
            get: (): UvCmColor => this.color,
            set: (newVal: UvCmColor): void => {
        ...
        },
        {
          ...
          editable: true,
          accessor: {
            get: (): UvGiLineWeight => this.lineWeight,
            set: (newVal: UvGiLineWeight): void => {
        ...
        },
        {
          ...
          editable: true,
          accessor: {
            get: (): UvCmTransparency => this.transparency,
            set: (newVal: UvCmTransparency): void => {
   ...
   * ```
   */
  private getLineType(): { type: UvGiStyleType; name: string } {
   ...
   *
   */
  private attachEntityInfo(target: UvGiEntity | null | undefined) {
    if (target) {
