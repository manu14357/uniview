 ...
 * const point = new UvDbPoint();
 * point.position = new UvGePoint3d(0, 0, 0);
 *
 * // Create a point at a specific location
 * const point2 = new UvDbPoint();
 * point2.position = new UvGePoint3d(10, 20, 5);
 *
 * // Access point properties
 * console.log(`Point position: ${point.position}`);
 * ```
 */
export class UvDbPoint extends UvDbEntity {
  /** The entity type name */
  static override typeName: string = 'Point'

  /** The underlying geometric point object */
  private _geo: UvGePoint3d
   ...
   * ```typescript
   * const point = new UvDbPoint();
   * point.position = new UvGePoint3d(5, 10, 0);
   * ```
   */
  constructor() {
    super()
    this._geo = new UvGePoint3d()
  }
   ...
   * @example
   * ```typescript
   * point.position = new UvGePoint3d(15, 25, 0);
   * ```
   */
  set position(value: UvGePointLike) {
  ...
  get geometricExtents(): UvGeBox3d {
    return new UvGeBox3d().expandByPoint(this._geo)
  }
  ...
  subGetOsnapPoints(
    ...
    if (UvDbOsnapMode.Node === osnapMode) {
