...
import { UvDbOsnapMode } from '../uniview-misc/uniview-db-osnap-mode'
...
export class UvDbLine extends UvDbCurve {
   ...
   * const line = new UvDbLine(
   *   new UvGePoint3d(0, 0, 0),
   *   new UvGePoint3d(100, 50, 0)
   * );
   * ```
   */
  constructor(start: UvGePoint3dLike, end: UvGePoint3dLike) {
    super()
    this._geo = new UvGeLine3d(start, end)
  }
   ...
   * @example
   * ```typescript
   * line.startPoint = new UvGePoint3d(5, 5, 0);
   * ```
   */
   ...
   * @example
   * ```typescript
   * line.endPoint = new UvGePoint3d(15, 15, 0);
   * ```
   */
  ...
  subWorldDraw(renderer: UvGiRenderer) {
    ...
    const points = [
      new UvGePoint3d(start.x, start.y, 0),
      new UvGePoint3d(end.x, end.y, 0)
    ]
