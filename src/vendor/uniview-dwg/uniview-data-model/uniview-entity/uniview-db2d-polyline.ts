 ...
 * - Each vertex is an instance of UvDb2dVertex.
...
export class UvDb2dPolyline extends UvDbCurve {
  ...
  constructor(
    ...
    this._geo = new UvGePolyline2d(polylineVertices, closed)
  }
   ...
   */
  set polyType(value: UvDbPoly2dType) {
  ...
  subGetGripPoints() {
    ...
    for (let i = 0; i < this._geo.numberOfVertices; ++i) {
      ...
      gripPoints.push(new UvGePoint3d(temp.x, temp.y, 0))
    }
    return gripPoints
  }
      ...
      case UvDbOsnapMode.EndPoint:
        {
          for (let i = 0; i < this._geo.numberOfVertices; ++i) {
            const temp = this._geo.getPointAt(i)
            snapPoints.push(new UvGePoint3d(temp.x, temp.y, 0))
          }
        }
        break
      default:
