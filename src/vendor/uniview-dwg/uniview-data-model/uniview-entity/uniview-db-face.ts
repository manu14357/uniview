 ...
 */
export class UvDbFace extends UvDbEntity {
  ...
  constructor() {
    super()
    this._vertices = [new UvGePoint3d(), new UvGePoint3d(), new UvGePoint3d()]
   ...
   * ```
   */
  getVertexAt(index: number): UvGePoint3d {
   ...
   * @example
   * ```typescript
   * face.setVertexAt(0, new UvGePoint3d(0, 0, 0));
   * face.setVertexAt(1, new UvGePoint3d(10, 0, 0));
   * face.setVertexAt(2, new UvGePoint3d(10, 5, 0));
   * face.setVertexAt(3, new UvGePoint3d(0, 5, 0));
   * ```
   */
  setVertexAt(index: number, point: UvGePointLike) {
    if (index < 0) this._vertices[0].copy(point)
    if (index >= 3) {
      if (this._vertices.length === 3) {
        this._vertices.push(new UvGePoint3d())
      }
