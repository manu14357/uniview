...
import { UvGiLineArrowStyle } from '@uniview/graphics'
 ...
 * const radialDim = new UvDbRadialDimension(
 *   new UvGePoint3d(0, 0, 0),    // Center point
 *   new UvGePoint3d(5, 0, 0),    // Point on curve
 *   2.0,                         // Leader length
 *   "5.0",                       // Dimension text
 ...
 * ```
 */
export class UvDbRadialDimension extends UvDbDimension {
  ...
  private _chordPoint: UvGePoint3d
   ...
   *   new UvGePoint3d(5, 0, 0),
   *   2.0
   * );
   *
   * // Create a radial dimension with custom text and style
   * const radialDim2 = new UvDbRadialDimension(
   *   new UvGePoint3d(10, 10, 0),
   ...
   */
  constructor(
    center: UvGePoint3dLike,
    chordPoint: UvGePoint3dLike,
    leaderLength: number,
    dimText: string | null = null,
    dimStyle: string | null = null
  ) {
    super()
    this._center = new UvGePoint3d().copy(center)
    this._chordPoint = new UvGePoint3d().copy(chordPoint)
   ...
   * @example
   * ```typescript
   * radialDim.center = new UvGePoint3d(0, 0, 0);
   * ```
   */
  set center(value: UvGePoint3d) {
   ...
   * @example
   * ```typescript
   * radialDim.chordPoint = new UvGePoint3d(5, 0, 0);
   * ```
   */
  set chordPoint(value: UvGePoint3d) {
   ...
   * @inheritdoc
   */
  protected getLineArrowStyle(_line: UvDbLine): UvGiLineArrowStyle | undefined {
    return {
