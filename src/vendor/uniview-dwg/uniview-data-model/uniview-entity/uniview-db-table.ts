...
import { UvDbTextStyleTableRecord } from '../uniview-database'
import { UvDbBlockReference } from './uniview-db-block-reference'
import { UvDbEntityProperties } from './uniview-db-entity-properties'

/**
 * Interface defining the properties of a table cell within an UvDbTable entity.
 *
 ...
 */
export interface UvDbTableCell {
...
const tempVector = /*@__PURE__*/ new UvGeVector3d()
 ...
 */
export class UvDbTable extends UvDbBlockReference {
  ...
  constructor(name: string, numRows: number, numColumns: number) {
    ...
    this._cells = new Array<UvDbTableCell>(numRows * numColumns)
  }
   ...
   */
  setCell(index: number, cell: UvDbTableCell) {
   ...
   */
  subWorldDraw(renderer: UvGiRenderer): UvGiEntity {
    let allRowHeights = 0,
      allColumnWidths = 0
        ...
        if (cell && !visited[cellIndex]) {
          ...
          if (cell.text) {
            const attachmentPoint =
              ...
              UvGiMTextAttachmentPoint.MiddleCenter
            ...
            const textStyle: UvGiTextStyle = this.getTextStyle(cell)
    ...
    const quaternion = new UvGeQuaternion()
    quaternion.setFromAxisAngle(UvGeVector3d.Z_AXIS, this.rotation)
   ...
   * @private
   */
  private getTextStyle(cell: UvDbTableCell): UvGiTextStyle {
    const textStyleTable = this.database.tables.textStyleTable
    let style: UvDbTextStyleTableRecord | undefined
    if (cell.textStyle) {
...
const _tmpMatrix = /*@__PURE__*/ new UvGeMatrix3d()
