 ...
 */
export type UvDbEntityPropertyType =
  | 'array'
  | 'string'
  | 'int'
  | 'float'
...
/**
 * Describes static metadata for a single property of an `UvDbEntity`.
 *
 ...
 * by {@link UvDbPropertyAccessor} in `UvDbEntityRuntimeProperty`.
 *
 ...
 */
export interface UvDbEntityProperty {
  /**
   ...
   */
  itemSchema?: UvDbEntityArrayItemSchema
}
 ...
 */
export interface UvDbEntityArrayItemSchema {
  /**
   * Property definitions for one array element.
   */
  properties: UvDbEntityProperty[]
}
 ...
 * to the underlying `UvDbEntity`. It allows the property panel to operate on
 ...
 * ```
 */
export interface UvDbPropertyAccessor<T = unknown> {
  /**
 ...
 *
 * While `UvDbEntityProperty` describes static metadata such as label, type,
 * and enum options, a runtime property additionally provides an
 * {@link UvDbPropertyAccessor} which allows the UI to read and modify the
 * live value stored inside the `UvDbEntity` instance.
 *
 ...
 * @example
 * ```ts
 * // Returned by UvDbCircle.properties():
 * {
 *   name: "centerX",
 *   label: "Center X",
 *   type: "float",
 ...
 * ```
 */
export interface UvDbEntityRuntimeProperty<T = unknown>
  extends UvDbEntityProperty {
  /**
   ...
   */
  accessor: UvDbPropertyAccessor<T>
}
 ...
 */
export type UvDbEntityPropertyGroupName = string
 ...
 *
 * Each group contains only metadata (`UvDbEntityProperty`) and not
 * accessors. Runtime access to values is handled by the resolved
 * structure returned from entity instances.
 */
export interface UvDbEntityPropertyGroup {
  /**
   * Name of the group, shown in the UI (e.g., "Geometry").
   */
  groupName: UvDbEntityPropertyGroupName

  /**
   * List of static property metadata belonging to this group.
   */
  properties: UvDbEntityRuntimeProperty[]
}

/**
 * Top-level metadata structure returned by `UvDbEntity.properties()`.
 *
 * This object describes:
 * - the entity type (e.g., `"UvDbLine"`, `"UvDbCircle"`),
 ...
 * {@link UvDbEntityRuntimeProperty}.
 */
export interface UvDbEntityProperties {
  /**
   * The name of the entity type (e.g., `"UvDbCircle"`).
   *
   ...
   */
  groups: UvDbEntityPropertyGroup[]
}
