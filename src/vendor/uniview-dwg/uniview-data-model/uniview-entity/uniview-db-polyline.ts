 ...
 */
interface UvDbPolylineVertex extends UvGePolyline2dVertex {
 ...
 * polyline.addVertexAt(0, new UvGePoint2d(0, 0));
 * polyline.addVertexAt(1, new UvGePoint2d(10, 0));
 * polyline.addVertexAt(2, new UvGePoint2d(10, 5));
 * polyline.addVertexAt(3, new UvGePoint2d(0, 5));
 ...
 * ```
 */
export class UvDbPolyline extends UvDbCurve {
  ...
  private _geo: UvGePolyline2d<UvDbPolylineVertex>
   ...
   * polyline.addVertexAt(1, new UvGePoint2d(5, 0), 0.5);
   *
   * // Add a vertex with custom width
   * polyline.addVertexAt(2, new UvGePoint2d(10, 0), 0, 2, 1);
   * ```
   */
  addVertexAt(
    index: number,
    ...
    const vertex: UvDbPolylineVertex = {
      x: pt.x,
      y: pt.y,
      bulge: bulge,
   ...
   * ```
   */
  getPoint2dAt(index: number): UvGePoint2d {
   ...
   * ```
   */
  getPoint3dAt(index: number): UvGePoint3d {
    const vertex = this.getPoint2dAt(index)
    return new UvGePoint3d(vertex.x, vertex.y, this._elevation)
  }
  ...
  subGetOsnapPoints(
    ...
    const endPoints = new Array<UvGePoint3d>()
    for (let index = 0; index < this.numberOfVertices; ++index) {
   ...
   *
   * The geometry group exposes properties via {@link UvDbPropertyAccessor} so
  ...
  subWorldDraw(renderer: UvGiRenderer) {
    ...
    tmp.forEach(point =>
      points.push(new UvGePoint3d().set(point.x, point.y, this.elevation))
    )
