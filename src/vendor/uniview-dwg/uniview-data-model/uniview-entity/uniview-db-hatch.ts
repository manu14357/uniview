 ...
 */
export enum UvDbHatchPatternType {
  /**
 ...
 * hatch.patternType = UvDbHatchPatternType.Predefined;
 * hatch.patternScale = 1.0;
 * hatch.patternAngle = 0;
 * hatch.hatchStyle = UvDbHatchStyle.Normal;
 *
 * // Add a loop to define the hatch boundary
 * const loop = new UvGeLoop2d();
 * loop.add(new UvGePoint2d(0, 0));
 * loop.add(new UvGePoint2d(10, 0));
 * loop.add(new UvGePoint2d(10, 5));
 * loop.add(new UvGePoint2d(0, 5));
 * hatch.add(loop);
 *
...
export class UvDbHatch extends UvDbEntity {
  ...
  private _definitionLines: UvGiHatchPatternLine[]
  /** The name of the hatch pattern */
  private _patternName: string
  /** The type of hatch pattern */
  private _patternType: UvDbHatchPatternType
  ...
  private _hatchStyle: UvDbHatchStyle
  ...
  constructor() {
    super()
    this._elevation = 0
    this._geo = new UvGeArea2d()
    this._isSolidFill = false
    this._definitionLines = []
    this._patternName = ''
    this._patternType = UvDbHatchPatternType.Predefined
    this._patternAngle = 0
    this._patternScale = 1
    this._hatchStyle = UvDbHatchStyle.Normal
  }
  ...
  set patternType(value: UvDbHatchPatternType) {
  ...
  set hatchStyle(value: UvDbHatchStyle) {
          ...
          properties: [
            {
              ...
              editable: true,
              options: [
                { label: UvDbHatchPatternType[0], value: 0 },
                { label: UvDbHatchPatternType[1], value: 1 },
                { label: UvDbHatchPatternType[2], value: 2 }
              ],
              accessor: {
                get: () => this.patternType,
                set: (v: UvDbHatchPatternType) => {
                  this.patternType = v
                }
              }
            },
