 ...
 */
export enum UvDbLeaderAnnotationType {
 ...
 * const leader = new UvDbLeader();
 * leader.appendVertex(new UvGePoint3d(0, 0, 0));
 * leader.appendVertex(new UvGePoint3d(5, 5, 0));
 * leader.appendVertex(new UvGePoint3d(10, 5, 0));
 * leader.hasArrowHead = true;
 * leader.hasHookLine = true;
 * leader.annoType = UvDbLeaderAnnotationType.MText;
 *
 ...
 * ```
 */
export class UvDbLeader extends UvDbCurve {
  ...
  private _splineGeo?: UvGeSpline3d
  ...
  private _annoType: UvDbLeaderAnnotationType
  ...
  constructor() {
    ...
    this._annoType = UvDbLeaderAnnotationType.NoAnnotation
  }
   ...
   * ```
   */
  set annoType(value: UvDbLeaderAnnotationType) {
   ...
   */
  appendVertex(point: UvGePoint3dLike) {
    this._vertices.push(new UvGePoint3d().copy(point))
    this._updated = true
  }
   ...
   */
  setVertexAt(index: number, point: UvGePoint3dLike) {
  ...
  private createSplineIfNeeded() {
    if (
      ...
      this._splineGeo = new UvGeSpline3d(this._vertices, 'Uniform')
      this._updated = false
    }
  }
}
