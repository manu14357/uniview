...
import { UvDbDatabase } from '../uniview-database'
import { UvDbObjectIterator } from '../uniview-misc'
 ...
 * derived from UvDbObject may be added, accessed, and removed. Entries in an UvDbDictionary must be unique.
 * Entries consist of a unique UvDbObject and string, which comprises the entry's key name. The key may be
 ...
 * @example
 * ```typescript
 * const dictionary = new UvDbDictionary<UvDbLayout>(database);
 * const layout = new UvDbLayout();
 * dictionary.setAt('MyLayout', layout);
 * const retrievedLayout = dictionary.getAt('MyLayout');
 * ```
 */
export class UvDbDictionary<
  TObjectType extends UvDbObject = UvDbObject
> extends UvDbObject {
  ...
  /**
   * Creates a new UvDbDictionary instance.
   *
   * @param db - The database this dictionary belongs to
   *
   * @example
   * ```typescript
   * const dictionary = new UvDbDictionary(database);
   * ```
   */
  ...
  setAt(key: string, value: TObjectType) {
    ...
    this.database.events.dictObjectSet.dispatch({
   ...
   *
   * @returns A new UvDbObjectIterator for this dictionary
   *
   * @example
   * ```typescript
   ...
   * ```
   */
  newIterator(): UvDbObjectIterator<TObjectType> {
