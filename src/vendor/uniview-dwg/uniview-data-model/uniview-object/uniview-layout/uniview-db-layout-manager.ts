...
import { uvdbHostApplicationServices, UvDbObjectId } from '../../uniview-base'
import { UvDbBlockTableRecord, UvDbDatabase } from '../../uniview-database'
import { UvDbLayout } from './uniview-db-layout'

/**
 * Event arguments for layout-related events.
 */
export interface UvDbLayoutEventArgs {
 ...
 */
export interface UvDbLayoutRenamedEventArgs {
  /** The layout involved in the event */
  layout: UvDbLayout
...
export class UvDbLayoutManager {
  ...
  public readonly events = {
    ...
    layoutCreated: new UvCmEventManager<UvDbLayoutEventArgs>(),
    /** Fired when the layout is removed */
    layoutRemoved: new UvCmEventManager<UvDbLayoutEventArgs>(),
    /** Fired when the layout is renamed */
    layoutRenamed: new UvCmEventManager<UvDbLayoutRenamedEventArgs>(),
    /** Fired when the layout is switched */
    layoutSwitched: new UvCmEventManager<UvDbLayoutEventArgs>()
  }
   ...
   * ```
   */
  findLayoutNamed(name: string, db?: UvDbDatabase) {
   ...
   * ```
   */
  setCurrentLayoutId(id: UvDbObjectId, db?: UvDbDatabase) {
   ...
   * ```
   */
  setCurrentLayoutBtrId(id: UvDbObjectId, db?: UvDbDatabase) {
    const currentDb = this.getWorkingDatabase(db)
   ...
   * ```
   */
  setCurrentLayout(name: string, db?: UvDbDatabase) {
    const currentDb = this.getWorkingDatabase(db)
   ...
   * ```
   */
  renameLayout(oldName: string, newName: string, db?: UvDbDatabase) {
    const currentDb = this.getWorkingDatabase(db)
   ...
   */
  layoutExists(name: string, db?: UvDbDatabase) {
   ...
   * @returns
   */
  deleteLayout(name: string, db?: UvDbDatabase) {
  ...
  /**
   * Create a new UvDbLayout object given a unique layout name.
   * @param name Input name to give new UvDbLayout object
   * @param db Input drawing database to use. Default is the current database.
   * @returns Return newly created layout and its associated block table record.
   */
  createLayout(name: string, db?: UvDbDatabase) {
    const currentDb = this.getWorkingDatabase(db)
    ...
    const btr = new UvDbBlockTableRecord()
   ...
   */
  getActiveLayout(db?: UvDbDatabase) {
    const currentDb = this.getWorkingDatabase(db)
    return currentDb.objects.layout.getBtrIdAt(currentDb.currentSpaceId)
  }

  private getWorkingDatabase(db?: UvDbDatabase) {
    return db || uvdbHostApplicationServices().workingDatabase
  }

  private setCurrentLayoutInternal(
    layout: UvDbLayout | undefined,
    currentDb: UvDbDatabase
  ) {
    if (layout) {
