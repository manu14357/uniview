import DxfParser, { ParsedDxf } from '@uniview/dxf-parser'

// Please don't modify the following two lines to import from '../uniview-database' and
// import from '../uniview-misc' so that treeshaking can include classes really needed.
import { UvDbDwgVersion } from '../uniview-database/uniview-db-dwg-version'
import { UvDbCodePage, dwgCodePageToEncoding } from '../uniview-misc/uniview-db-code-page'
 ...
 */
export interface UvDbDxfHeaderInfo {
  version: UvDbDwgVersion | null
...
export class UvDbDxfParser {
   ...
   */
  private getDxfInfoFromBuffer(buffer: ArrayBuffer): UvDbDxfHeaderInfo {
    const chunkSize = 64 * 1024 // 64 KB
    const decoder = new TextDecoder('utf-8')
    let offset = 0
    let leftover = ''
    let version: UvDbDwgVersion | null = null
    ...
    while (offset < buffer.byteLength) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (inHeader && line === '$ACADVER') {
          ...
          if (value) version = new UvDbDwgVersion(value)
        }
        // parse code page
        else if (inHeader && line === '$DWGCODEPAGE') {
          const value: string | undefined = lines[i + 2]?.trim()
          if (value) {
            const codePage = UvDbCodePage[value as keyof typeof UvDbCodePage]
