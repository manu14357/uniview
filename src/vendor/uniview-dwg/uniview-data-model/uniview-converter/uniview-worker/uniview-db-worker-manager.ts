...
export interface UvDbWorkerConfig {
...
export interface UvDbWorkerResult<TOutput = unknown> {
  success: boolean
  data?: TOutput
  error?: string
  duration: number
...
export interface UvDbWorkerInstance {
  worker: Worker
  isBusy: boolean
  id: string
  createdAt: Date
 ...
 * Simple worker framework
 */
export class UvDbWorkerManager {
  private config: Required<UvDbWorkerConfig>
  private taskId = 0
  private workers = new Map<string, UvDbWorkerInstance>()
  private pendingTasks = new Map<
    string,
    {
      resolve: (value: UvDbWorkerResult) => void
      reject: (error: Error) => void
      timeout: ReturnType<typeof setTimeout>
    }
  >()

  constructor(config: UvDbWorkerConfig) {
    this.config = {
  ...
  async execute<TInput, TOutput>(
    input: TInput,
    workerUrl?: string
  ): Promise<UvDbWorkerResult<TOutput>> {
  ...
  private async executeInWorker<TInput, TOutput>(
    ...
    return new Promise<UvDbWorkerResult<TOutput>>((resolve, reject) => {
      ...
      this.pendingTasks.set(taskId, {
        resolve: result => {
          ...
          this.releaseWorker(worker)
          resolve(result as UvDbWorkerResult<TOutput>)
        },
        reject: error => {
          clearTimeout(timeout)
          this.releaseWorker(worker)
  ...
  private getAvailableWorker(workerUrl: string | URL): Worker {
    ...
    if (this.workers.size < this.config.maxConcurrentWorkers) {
      ...
      const instance: UvDbWorkerInstance = {
        worker,
        isBusy: true,
        id,
        createdAt: new Date()
 ...
 */
export class UvDbWorkerApi {
  private framework: UvDbWorkerManager

  constructor(config: UvDbWorkerConfig) {
    this.framework = new UvDbWorkerManager(config)
  }
 ...
 */
export function createWorkerApi(config: UvDbWorkerConfig): UvDbWorkerApi {
  return new UvDbWorkerApi(config)
}
