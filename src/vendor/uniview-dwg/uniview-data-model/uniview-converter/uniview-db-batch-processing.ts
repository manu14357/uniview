...
type UvDbChunkProcessingCallback = (start: number, end: number) => Promise<void>

// Callback function to execute when all of chunks are processed
type UvDbChunkProcessingCompleteCallback = () => void | Promise<void>
 ...
 * @example
 * ```typescript
 * const batchProcessor = new UvDbBatchProcessing(1000, 10, 50);
 ...
 * ```
 */
export class UvDbBatchProcessing {
  ...
  /**
   * Creates a new UvDbBatchProcessing instance.
   *
  ...
  public async processChunk(
    callback: UvDbChunkProcessingCallback,
    onComplete?: UvDbChunkProcessingCompleteCallback
  ) {
    if (this._count <= 0) {
      await onComplete?.()
      return
