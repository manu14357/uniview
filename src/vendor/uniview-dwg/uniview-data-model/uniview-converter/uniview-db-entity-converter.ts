import { UvCmTransparency } from '@uniview/common'
import {
  ArcEntity,
  AttdefEntity,
  AttributeEntity,
...
import { CircleEntity } from '@uniview/dxf-parser'
import {
  ArcEdge,
  BoundaryPathEdge,
  EdgeBoundaryPath,
...
/**
 * Converts DXF entities to UvDbEntity objects.
 *
 ...
 * UvDbEntity objects. It handles the conversion of geometric data,
...
export class UvDbEntityConverter {
   ...
   * ```
   */
  convert(entity: CommonDxfEntity): UvDbEntity | null {
   ...
   * ```
   */
  private createEntity(entity: CommonDxfEntity): UvDbEntity | null {
  ...
  /**
   * Converts a DXF 3DFACE entity to an UvDbFace.
   *
   * @param face - The DXF 3DFace entity to convert
   * @returns The converted UvDbFace entity
   */
  ...
  /**
   * Converts a DXF arc entity to an UvDbArc.
   *
   * @param arc - The DXF arc entity to convert
   * @returns The converted UvDbArc entity
   *
   * @example
   * ```typescript
  ...
  private convertArc(arc: ArcEntity) {
    const dbEntity = new UvDbArc(
      arc.center,
      arc.radius,
      UvGeMathUtil.degToRad(arc.startAngle),
      UvGeMathUtil.degToRad(arc.endAngle),
  ...
  private convertAttributeCommon(
    ...
    dbAttrib.isInvisible =
      (attrib.attributeFlag & UvDbAttributeFlags.Invisible) !== 0
    dbAttrib.isConst = (attrib.attributeFlag & UvDbAttributeFlags.Const) !== 0
    dbAttrib.isVerifiable =
      (attrib.attributeFlag & UvDbAttributeFlags.Verifiable) !== 0
    dbAttrib.isPreset = (attrib.attributeFlag & UvDbAttributeFlags.Preset) !== 0
  ...
  private convertAttribute(attrib: AttributeEntity) {
    ...
    dbAttrib.horizontalMode =
      attrib.horizontalJustification as UvDbTextHorizontalMode
    dbAttrib.verticalMode = attrib.verticalJustification as UvDbTextVerticalMode
  ...
  private convertAttributeDefinition(attrib: AttdefEntity) {
    ...
    dbAttDef.horizontalMode = attrib.halign as unknown as UvDbTextHorizontalMode
    dbAttDef.verticalMode = attrib.valign as unknown as UvDbTextVerticalMode
  ...
  /**
   * Converts a DXF circle entity to an UvDbCircle.
   *
   * @param circle - The DXF circle entity to convert
   * @returns The converted UvDbCircle entity
   *
   * @example
   * ```typescript
  ...
  /**
   * Converts a DXF ellipse entity to an UvDbEllipse.
   *
   * @param ellipse - The DXF ellipse entity to convert
   * @returns The converted UvDbEllipse entity
   *
   * @example
   * ```typescript
  ...
  private convertLine(line: LineEntity) {
    ...
    const dbEntity = new UvDbLine(
      new UvGePoint3d(start.x, start.y, start.z || 0),
      new UvGePoint3d(end.x, end.y, end.z || 0)
    )
    return dbEntity
  }
          ...
          if (edge.type == 1) {
            ...
            loop.add(
              new UvGeCircArc2d(
                arc.center,
                arc.radius,
                UvGeMathUtil.degToRad(arc.startAngle || 0),
                UvGeMathUtil.degToRad(arc.endAngle || 0),
                !arc.isCCW
              )
            )
          } else if (edge.type == 3) {
            ...
            let startAngle = UvGeMathUtil.degToRad(ellipse.startAngle || 0)
            let endAngle = UvGeMathUtil.degToRad(ellipse.endAngle || 0)
  ...
  private convertText(text: TextEntity) {
    ...
    dbEntity.rotation = UvGeMathUtil.degToRad(text.rotation || 0)
  ...
  private convertMText(mtext: MTextEntity) {
    ...
    dbEntity.rotation = UvGeMathUtil.degToRad(mtext.rotation || 0)
  ...
  private convertDimension(dimension: DimensionEntityCommon) {
    if (
      ...
      dbEntity.rotation = UvGeMathUtil.degToRad(entity.rotationAngle || 0)
  ...
  private processImage(image: ImageEntity, dbImage: UvDbRasterImage) {
  ...
  private processWipeout(wipeout: WipeoutEntity, dbWipeout: UvDbWipeout) {
    ...
    wipeout.boundary.forEach(point => {
      dbWipeout.clipBoundary.push(new UvGePoint2d(point))
    })
    dbWipeout.clipBoundaryType =
      wipeout.boundaryType as unknown as UvDbRasterImageClipBoundaryType
  ...
  private convertBlockReference(blockReference: InsertEntity) {
    ...
    dbBlockReference.rotation =
      blockReference.rotation != null
        ? UvGeMathUtil.degToRad(blockReference.rotation)
        : 0
  ...
  /**
   * Processes common attributes from a DXF entity to an UvDbEntity.
   *
   ...
   * DXF entity to the corresponding UvDbEntity.
   *
   * @param entity - The source DXF entity
   * @param dbEntity - The target UvDbEntity to populate
   *
   * @example
   * ```typescript
   ...
   * ```
   */
  private processCommonAttrs(entity: CommonDxfEntity, dbEntity: UvDbEntity) {
    dbEntity.layer = entity.layer || '0'
    // I found some dxf file may have entity without handle. If so, let's use objectId
    // created by UvDbObject constructor instead.
    ...
    if (entity.transparency != null) {
      dbEntity.transparency = UvCmTransparency.deserialize(entity.transparency)
    }
  }
   ...
   * an array of UvGePoint3dLike objects. It automatically detects whether
   ...
   * @returns Array of UvGePoint3dLike objects, or undefined if the conversion fails
   *
   * @example
   * ```typescript
