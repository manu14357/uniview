 ...
 *
 * This class extends UvDbDatabaseConverter so that it can leverage the existing
 ...
 * it.
 */
export class UvDbRegenerator extends UvDbDatabaseConverter<UvDbDatabase> {
  ...
  protected processObjects() {
    ...
    for (const layout of layouts) {
      this._database.events.dictObjectSet.dispatch({
   ...
   */
  private groupAndFlattenByType(entities: UvDbEntity[]) {
    const groups: Record<string, UvDbEntity[]> = {}
    const order: string[] = []
  ...
  private triggerEvents(btr: UvDbBlockTableRecord, entities: UvDbEntity[]) {
    ...
    if (btr.isModelSpace || btr.isPaperSpace) {
