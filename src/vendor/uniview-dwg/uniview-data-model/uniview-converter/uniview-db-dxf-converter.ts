...
import { DxfBlock } from '@uniview/dxf-parser'
import { CommonDxfEntity } from '@uniview/dxf-parser'
import { ImageDefDXFObject } from '@uniview/dxf-parser'
import { LayoutDXFObject } from '@uniview/dxf-parser'
import { CommonDxfTableEntry } from '@uniview/dxf-parser'
import {
  UvGiDefaultLightingType,
  UvGiOrthographicType,
  UvGiRenderMode
...
import {
  ...
  UvDbDatabase,
  UvDbDatabaseConverterConfig,
  UvDbDimStyleTableRecord,
  UvDbDimStyleTableRecordAttrs,
  UvDbDimTextHorizontal,
  UvDbDimTextVertical,
  UvDbDimVerticalJustification,
  UvDbDimZeroSuppression,
  UvDbDimZeroSuppressionAngular,
  UvDbLayerTableRecord,
  UvDbLinetypeTableRecord,
  UvDbSymbolTableRecord,
  UvDbTextStyleTableRecord,
  UvDbViewportTableRecord
} from '../uniview-database'
import {
  UvDbConversionProgressCallback,
  UvDbDatabaseConverter
} from '../uniview-database/uniview-db-database-converter'
import { UvDbAttribute, UvDbBlockReference, UvDbEntity } from '../uniview-entity'
import { UvDbBatchProcessing } from './uniview-db-batch-processing'
import { UvDbDxfParser } from './uniview-db-dxf-parser'
import { UvDbEntityConverter } from './uniview-db-entity-converter'
import { UvDbObjectConverter } from './uniview-db-object-converter'
import { createWorkerApi } from './uniview-worker'

/**
 * Default database converter for DXF files.
 *
 * This class extends UvDbDatabaseConverter to provide specialized functionality
 * for converting DXF (Drawing Exchange Format) files into UvDbDatabase objects.
 ...
 * ```
 */
export class UvDbDxfConverter extends UvDbDatabaseConverter<ParsedDxf> {
  ...
  protected async processEntities(
    ...
    const converter = new UvDbEntityConverter()

    // Create an instance of UvDbBatchProcessing
    let entities = dxf.entities
    const entityCount = entities.length
    const batchProcessor = new UvDbBatchProcessing(
      entityCount,
    ...
    const attributeMap: Map<UvDbObjectId, UvDbAttribute[]> = new Map()
    for (let i = 0; i < entityCount; i++) {
      ...
      if (entity.type === 'ATTRIB') {
        ...
        if (dbEntity && dbEntity.ownerId && dbEntity.ownerId !== '0') {
          ...
          attributes.push(dbEntity as UvDbAttribute)
        }
      }
    }
            ...
            if (entity.type === 'INSERT') {
              ...
              if (attributes && attributes.length > 0) {
                attributes.forEach(attribute => {
                  ;(dbEntity as UvDbBlockReference).appendAttributes(attribute)
                })
              }
            }
   ...
   * corresponding UvDbBlockTableRecord objects in the database.
   *
   ...
   * ```
   */
  protected processBlocks(model: ParsedDxf, db: UvDbDatabase) {
    ...
    for (const [name, block] of Object.entries(blocks)) {
      ...
      if (block.entities) {
        ...
        // Process paper space block definition. Entities in model space are
        // handled in method processEntities
        if (dbBlock.isPaperSpace) {
   ...
   * ```
   */
  protected processHeader(model: ParsedDxf, db: UvDbDatabase) {
   ...
   *
   * This method creates UvDbBlockTableRecord objects for each block record
   ...
   * ```
   */
  protected processBlockTables(dxf: ParsedDxf, db: UvDbDatabase) {
    const btrs = dxf.tables.BLOCK_RECORD?.entries
    if (btrs && btrs.length > 0) {
      db.tables.blockTable.removeAll()
      btrs.forEach(btr => {
        const dbBlock = new UvDbBlockTableRecord()
   ...
   * ```
   */
  protected processObjects(model: ParsedDxf, db: UvDbDatabase) {
    const objects = model.objects.byName
    const objectConverter = new UvDbObjectConverter()
   ...
   *
   * This method creates UvDbViewportTableRecord objects for each viewport
   ...
   * ```
   */
  protected processViewports(model: ParsedDxf, db: UvDbDatabase) {
    const viewports = model.tables?.VPORT?.entries
    if (viewports && viewports.length > 0) {
      viewports.forEach(item => {
        const record = new UvDbViewportTableRecord()
        ...
        if (item.renderMode) {
          record.gsView.renderMode =
            item.renderMode as unknown as UvGiRenderMode
        }
        if (item.viewMode) {
        ...
        if (item.orthographicType) {
          record.gsView.orthographicType =
            item.orthographicType as unknown as UvGiOrthographicType
        }
        ...
        if (item.defaultLightingType) {
          record.gsView.defaultLightingType =
            item.defaultLightingType as unknown as UvGiDefaultLightingType
        }
        if (item.brightness) {
   ...
   *
   * This method creates UvDbLayerTableRecord objects for each layer
   * defined in the DXF tables section, including their properties like
   ...
   * ```
   */
  protected processLayers(model: ParsedDxf, db: UvDbDatabase) {
   ...
   *
   * This method creates UvDbLinetypeTableRecord objects for each linetype
   * defined in the DXF tables section.
   *
   ...
   * ```
   */
  protected processLineTypes(model: ParsedDxf, db: UvDbDatabase) {
    const lineTypes = model.tables?.LTYPE?.entries
    if (lineTypes && lineTypes.length > 0) {
      lineTypes.forEach(item => {
        const record = new UvDbLinetypeTableRecord(item)
        this.processCommonTableEntryAttrs(item, record)
   ...
   *
   * This method creates UvDbTextStyleTableRecord objects for each text style
   * defined in the DXF tables section.
   *
   ...
   * ```
   */
  protected processTextStyles(model: ParsedDxf, db: UvDbDatabase) {
    const textStyles = model.tables.STYLE?.entries
    if (textStyles && textStyles.length > 0) {
      textStyles.forEach(item => {
        const record = new UvDbTextStyleTableRecord(item)
        this.processCommonTableEntryAttrs(item, record)
   ...
   *
   * This method creates UvDbDimStyleTableRecord objects for each dimension style
   ...
   * ```
   */
  protected processDimStyles(model: ParsedDxf, db: UvDbDatabase) {
    const dimStyles = model.tables.DIMSTYLE?.entries
    if (dimStyles && dimStyles.length > 0) {
      dimStyles.forEach(item => {
        const attrs: UvDbDimStyleTableRecordAttrs = {
          name: item.name,
          ...
          dimtad: item.DIMTAD as unknown as UvDbDimTextVertical.Center,
          dimzin: item.DIMZIN as unknown as UvDbDimZeroSuppression.Feet,
          dimazin:
            item.DIMAZIN as unknown as UvDbDimZeroSuppressionAngular.None,
          dimalt: item.DIMALT,
          ...
          dimjust: item.DIMJUST as unknown as UvDbDimTextHorizontal.Center,
          dimsd1: item.DIMSD1,
          dimsd2: item.DIMSD2,
          dimtolj:
            item.DIMTOLJ as unknown as UvDbDimVerticalJustification.Bottom,
          dimtzin: item.DIMTZIN as unknown as UvDbDimZeroSuppression.Feet,
          dimaltz: item.DIMALTZ as unknown as UvDbDimZeroSuppression.Feet,
          dimalttz: item.DIMALTTZ as unknown as UvDbDimZeroSuppression.Feet,
        ...
        const record = new UvDbDimStyleTableRecord(attrs)
        this.processCommonTableEntryAttrs(item, record)
   ...
   * @param dbEntry - UvDbSymbolTableRecord to populate with the data
   *
   * @example
   * ```typescript
  ...
  private processCommonTableEntryAttrs(
    ...
    dbEntry: UvDbSymbolTableRecord
  ) {
    dbEntry.name = entry.name
    // All of objects in drawing database should have handle. However, some VPORT objects in DXF file
    // don't have handle. In this time, just use objectId created in constructor of UvDbObject.
    // https://github.com/manu14357/uniview/issues/101
