...
import { UvGeMatrix3d, UvGeVector3d } from '@uniview/geometry'
...
export class UvDbRenderingCache {
  ...
  static get instance() {
    if (!this._instance) {
      this._instance = new UvDbRenderingCache()
    }
    return this._instance
  }

  /**
   * Creates a new UvDbRenderingCache instance.
   *
   * @example
   * ```typescript
   * const cache = new UvDbRenderingCache();
   * ```
   */
  constructor() {
   ...
   * ```
   */
  set(key: string, group: UvGiEntity) {
    ...
    if (blockTableRecord != null) {
      ...
      if (this.has(key)) {
        ...
        if (block && cache) {
          ...
          if (
            UvDbBlockTableRecord.name &&
            !UvDbBlockTableRecord.name.startsWith('*U')
          ) {
            this.set(key, block)
          }
        }
  ...
  private attachEntityInfo(target: UvGiEntity, source: UvDbEntity) {
...
const _tmpColor = /*@__PURE__*/ new UvCmColor()
