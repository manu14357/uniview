 ...
 */
const DEFAULT_VIEW: UvGiView = {
  center: new UvGePoint2d(),
  viewDirectionFromTarget: new UvGePoint3d(0, 0, 1),
  viewTarget: new UvGePoint3d(0, 0, 0),
  lensLength: 500,
  ...
  frozenLayers: [],
  styleSheet: '',
  renderMode: UvGiRenderMode.OPTIMIZED_2D,
  viewMode: 0,
  ucsIconSetting: 0,
  ucsOrigin: new UvGePoint3d(0, 0, 0),
  ucsXAxis: new UvGePoint3d(1, 0, 0),
  ucsYAxis: new UvGePoint3d(0, 1, 0),
  orthographicType: UvGiOrthographicType.TOP,
  shadePlotSetting: 0,
  shadePlotObjectId: undefined,
  visualStyleObjectId: undefined,
  isDefaultLightingOn: false,
  defaultLightingType: UvGiDefaultLightingType.ONE_DISTANT_LIGHT,
  brightness: 0,
  contrast: 0,
...
export class UvDbViewportTableRecord extends UvDbSymbolTableRecord {
  ...
  private _gsView: UvGiView

  /**
   * Creates a new UvDbViewportTableRecord instance.
   *
   * @example
   * ```typescript
   * const viewportRecord = new UvDbViewportTableRecord();
  ...
  constructor() {
    ...
    this._center = new UvGePoint2d()
    this._lowerLeftCorner = new UvGePoint2d(0, 0)
    this._upperRightCorner = new UvGePoint2d(1, 1)
    this._snapBase = new UvGePoint2d(0, 0)
    this._snapAngle = 0
    this._snapSpacing = new UvGePoint2d(0, 0)
    this._standardFlag = 0
    this._gridSpacing = new UvGePoint2d()
   ...
   * viewportRecord.lowerLeftCorner = new UvGePoint2d(0.25, 0.25);
   * ```
   */
  get lowerLeftCorner() {
    return this._lowerLeftCorner
  }
  set lowerLeftCorner(value: UvGePoint2d) {
  ...
  set upperRightCorner(value: UvGePoint2d) {
  ...
  set snapBase(value: UvGePoint2d) {
  ...
  /**
   * An UvGePoint2d in which the X value represents the X spacing of the snap grid and the Y value
   * represents the Y spacing of the snap grid. Both values are in drawing units.
   */
  get snapIncrements() {
    return this._snapSpacing
  }
  set snapIncrements(value: UvGePoint2d) {
  ...
  /**
   * An UvGePoint2d in which the X value represents the X spacing (in drawing units) of the grid and
   * the Y value represents the Y spacing of the grid.
   */
  get gridIncrements() {
    return this._gridSpacing
  }
  set gridIncrements(value: UvGePoint2d) {
  ...
  /**
   * The UvGiView associated with this viewport table record
   */
  get gsView() {
    return this._gsView
  }
