import { UvCmColor, UvCmTransparency, defaults } from '@uniview/common'
import { UvGiLineStyle, UvGiLineWeight } from '@uniview/graphics'
 ...
 *
 * Extends the base UvDbSymbolTableRecordAttrs interface and adds layer-specific
 * properties such as color, visibility, linetype, and other layer settings.
 */
export interface UvDbLayerTableRecordAttrs extends UvDbSymbolTableRecordAttrs {
 ...
 * @example
 * ```typescript
 * const layer = new UvDbLayerTableRecord({
 *   name: 'MyLayer',
 *   color: new UvCmColor(255, 0, 0), // Red
 *   isOff: false,
 *   isPlottable: true
 * });
 * ```
 */
export class UvDbLayerTableRecord extends UvDbSymbolTableRecord<UvDbLayerTableRecordAttrs> {
  /**
   * Creates a new UvDbLayerTableRecord instance.
   *
   ...
   */
  constructor(
    attrs?: Partial<UvDbLayerTableRecordAttrs>,
    defaultAttrs?: Partial<UvDbLayerTableRecordAttrs>
  ) {
    attrs = attrs || {}
    defaults(attrs, {
      color: new UvCmColor(),
   ...
   * layer.color = new UvCmColor(255, 0, 0); // Red
   * ```
   */
  get color() {
  ...
  set isFrozen(value: boolean) {
    if (value) {
      this.standardFlags = this.standardFlags | 0x01
    } else {
      this.standardFlags = this.standardFlags & ~0x01
    }
  }
   ...
   */
  get lineStyle(): UvGiLineStyle | undefined {
