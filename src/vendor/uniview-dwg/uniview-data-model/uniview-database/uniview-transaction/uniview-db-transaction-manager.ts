import { UvDbTransaction } from './uniview-db-transaction'

/**
 * Manages database transactions.
 *
 * This class is the TypeScript equivalent of ObjectARX UvDbTransactionManager.
 * It supports nested transactions and object open tracking.
 *
 * A transaction must be explicitly committed or aborted.
 */
export class UvDbTransactionManager {
  /** Stack of active transactions */
  private readonly transactionStack: UvDbTransaction[] = []

  /**
   * Starts a new transaction and pushes it onto the stack.
   *
   * Equivalent to UvDbTransactionManager::startTransaction().
   *
   * @returns The newly created transaction
   */
  startTransaction(): UvDbTransaction {
    const tr = new UvDbTransaction()
   ...
   */
  currentTransaction(): UvDbTransaction | undefined {
