import { UvDbLine, UvGePoint3dLike, UvGiLineWeight } from '@uniview/data-model'
...
import { blueColor } from '../uniview-util'
import { registerMeasurementCleanup } from './uniview-ap-clear-measurements-cmd'

/** Returns the 2D Euclidean distance between two world points. */
function calcDist(p1: UvGePoint3dLike, p2: UvGePoint3dLike): number {
  const dx = p2.x - p1.x
  const dy = p2.y - p1.y
 ...
 */
export class UvApMeasureDistanceJig extends UvEdPreviewJig<UvGePoint3dLike> {
  private _line: UvDbLine
  private _p1: UvGePoint3dLike
  private _view: UvEdBaseView
  private _badge: HTMLDivElement

  constructor(view: UvEdBaseView, p1: UvGePoint3dLike) {
    super(view)
    this._p1 = p1
    this._view = view
    this._line = new UvDbLine(p1, p1)
    this._line.color = blueColor()
    this._line.lineWeight = UvGiLineWeight.LineWeight070
 ...
 */
export class UvApMeasureDistanceCmd extends UvEdCommand {
  constructor() {
    super()
