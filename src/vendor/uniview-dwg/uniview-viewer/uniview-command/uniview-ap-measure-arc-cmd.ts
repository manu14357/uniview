import { UvDbArc, UvDbCircle, UvDbLine, UvGePoint3dLike } from '@uniview/data-model'
 ...
 */
class UvApArcSnapJig extends UvEdPreviewJig<UvGePoint3dLike> {
  private _dummy: UvDbLine
  private _indicator: HTMLDivElement
  private _ctx: UvApContext
  private _onSnap: (geom: CircleGeom | null, snapped: UvGePoint3dLike | null) => void

  constructor(
    context: UvApContext,
    onSnap: (geom: CircleGeom | null, snapped: UvGePoint3dLike | null) => void
  ) {
    super(context.view)
    this._ctx = context
    this._onSnap = onSnap
  ...
  update(p: UvGePoint3dLike) {
    ...
    for (const hit of hits) {
      ...
      if (entity instanceof UvDbCircle) {
        geom = { cx: entity.center.x, cy: entity.center.y, r: entity.radius }
      } else if (entity instanceof UvDbArc) {
        geom = { cx: entity.center.x, cy: entity.center.y, r: entity.radius }
      }
 ...
 */
class UvApArcEndSnapJig extends UvEdPreviewJig<UvGePoint3dLike> {
  private _dummy: UvDbLine
  private _indicator: HTMLDivElement
  private _ctx: UvApContext
  private _geom: CircleGeom
  private _onMove: (snapped: UvGePoint3dLike) => void

  constructor(
    context: UvApContext,
    geom: CircleGeom,
    onMove: (snapped: UvGePoint3dLike) => void
  ) {
    super(context.view)
    this._ctx = context
    this._geom = geom
 ...
 */
export class UvApMeasureArcCmd extends UvEdCommand {
  constructor() {
    super()
  ...
  async execute(context: UvApContext) {
    const editor = UvApDocManager.instance.editor
    ...
    let snappedStart: UvGePoint3dLike | null = null

    const snapJig = new UvApArcSnapJig(context, (geom, snapped) => {
      snapGeom = geom
      snappedStart = snapped
    })

    const p1Prompt = new UvEdPromptPointOptions(
      UvApI18n.t('jig.measureArc.startPoint')
    )
    ...
    const onMove = (snapped: UvGePoint3dLike) => {
    ...
    const p2Prompt = new UvEdPromptPointOptions(
      UvApI18n.t('jig.measureArc.endPoint')
    )
    p2Prompt.jig = new UvApArcEndSnapJig(context, geom, onMove)

    let p2Raw: UvGePoint3dLike
    try {
