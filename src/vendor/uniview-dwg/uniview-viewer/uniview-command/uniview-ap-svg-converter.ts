import { UvSvgRenderer } from '@uniview/svg-renderer'
 ...
 * @example
 * ```typescript
 * const converter = new UvApSvgConverter();
 *
 * // Convert and download current drawing as SVG
 * converter.convert();
 * ```
 */
export class UvApSvgConverter {
  /**
  ...
  convert() {
    const entities =
      UvApDocManager.instance.curDocument.database.tables.blockTable.modelSpace.newIterator()
