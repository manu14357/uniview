 ...
 */
class UvApMeasureAreaJig extends UvEdPreviewJig<UvGePoint3dLike> {
  private _line: UvDbLine
  private _onMove: (p: UvGePoint3dLike) => void

  constructor(
    view: UvEdBaseView,
    from: UvGePoint3dLike,
    onMove: (p: UvGePoint3dLike) => void
  ) {
    super(view)
    this._line = new UvDbLine(from, from)
...
function shoelaceArea(pts: UvGePoint3dLike[]): number {
  let area = 0
  const n = pts.length
  for (let i = 0; i < n; i++) {
    const j = (i + 1) % n
...
function centroid(pts: UvGePoint3dLike[]): { x: number; y: number } {
 ...
 */
export class UvApMeasureAreaCmd extends UvEdCommand {
  constructor() {
    super()
  ...
  async execute(context: UvApContext) {
    ...
    const drawPolygon = (cursor?: UvGePoint3dLike) => {
    ...
    const p1 = await editor.getPoint(
      new UvEdPromptPointOptions(UvApI18n.t('jig.measureArea.firstPoint'))
    )
    points.push(p1)
    drawPolygon()
    ...
    try {
      while (points.length < 50) {
        const prompt = new UvEdPromptPointOptions(
          UvApI18n.t('jig.measureArea.nextPoint')
        )
        prompt.useBasePoint = true

        const onMove = (cursor: UvGePoint3dLike) => {
        ...
        prompt.jig = new UvApMeasureAreaJig(
          context.view,
        ...
        if (points.length >= 3) {
          ...
          const snap = (anchor: UvGePoint3dLike) => {
