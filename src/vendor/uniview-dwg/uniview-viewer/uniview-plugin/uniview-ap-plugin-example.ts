...
import { UvEdCommand } from '../uniview-editor/uniview-command/uniview-ed-command'
 ...
 *
 * This example shows:
 * - How to implement the UvApPlugin interface
 * - How to register custom commands in the onLoad hook
 * - How to clean up resources in the onUnload hook
 *
 * @example
 * ```typescript
 * import { UvApDocManager } from '@uniview/viewer';
 * import { ExamplePlugin } from './uniview-ap-plugin-example';
 *
 * // Load the plugin
 * const plugin = new ExamplePlugin();
 * await UvApDocManager.instance.pluginManager.loadPlugin(plugin);
 *
 ...
 * await UvApDocManager.instance.pluginManager.unloadPlugin('ExamplePlugin');
 * ```
 */
 ...
 */
class UvApHelloCommand extends UvEdCommand {
 ...
 */
export class UvApExamplePlugin implements UvApPlugin {
  name = 'ExamplePlugin'
  version = '1.0.0'
   ...
   */
  onLoad(_context: UvApContext, commandManager: UvEdCommandStack): void {
    // Register a custom command
    commandManager.addCommand(
      'USER', // Command group
      'HELLO', // Global command name
      'Hello', // Local command name
      new UvApHelloCommand()
    )
   ...
   */
  onUnload(_context: UvApContext, commandManager: UvEdCommandStack): void {
