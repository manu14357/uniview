...
import { UvApPlugin } from './uniview-ap-plugin'
 ...
 * @example
 * ```typescript
 * const pluginManager = UvApDocManager.instance.pluginManager;
 *
 * // Load a plugin
 ...
 * ```
 */
export class UvApPluginManager {
  /** Map of loaded plugins by name */
  private _plugins: Map<string, UvApPlugin>
   ...
   */
  constructor(context: UvApContext, commandManager: UvEdCommandStack) {
   ...
   * ```
   */
  async loadPlugin(plugin: UvApPlugin): Promise<void> {
    const pluginName = plugin.name

    if (!pluginName) {
      throw new Error('[UvApPluginManager] Plugin name is required')
    }

    if (this._plugins.has(pluginName)) {
      throw new Error(
        `[UvApPluginManager] Plugin '${pluginName}' is already loaded`
      )
    }
    ...
    try {
      ...
      throw new Error(
        `[UvApPluginManager] Failed to load plugin '${pluginName}': ${error}`
      )
    }
  ...
  async unloadPlugin(pluginName: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        `[UvApPluginManager] Error unloading plugin '${pluginName}':`,
        error
      )
    }
   ...
   */
  getPlugin(pluginName: string): UvApPlugin | undefined {
  ...
  async loadPluginsFromConfig(
    plugins: Array<UvApPlugin | (() => UvApPlugin)>,
  ...
  async loadPluginsFromFolder(
    ...
    if (pluginList.length === 0) {
      console.warn(
        '[UvApPluginManager] No plugin list provided. Cannot load plugins from folder without a list of files.'
      )
      return { loaded, failed }
    }
    ...
    for (const pluginFile of pluginList) {
      try {
        ...
        if (!plugin || typeof plugin.onLoad !== 'function') {
          throw new Error(
            `Invalid plugin in ${pluginFile}. Plugin must implement UvApPlugin interface.`
          )
        }
