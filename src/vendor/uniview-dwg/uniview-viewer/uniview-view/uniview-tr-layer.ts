 ...
 */
export type UvTrLayerStats = UvTrBatchedGroupStats & {
 ...
 * ```typescript
 * // Create a new layer
 * const layer = new UvTrLayer('Dimensions');
 *
 * // Add entities to the layer
 * const line = new UvTrLine(startPoint, endPoint);
 * layer.add(line);
 *
...
export class UvTrLayer {
   ...
   */
  private _group: UvTrBatchedGroup
   ...
   */
  constructor(layer: UvEdLayerInfo) {
    this._group = new UvTrBatchedGroup()
   ...
   * return false.
   */
  hasEntity(objectId: UvDbObjectId) {
   ...
   */
  removeEntity(objectId: UvDbObjectId): boolean {
   ...
   */
  updateEntity(entity: UvTrEntity): boolean {
    if (!entity.objectId) {
      return false
    }
