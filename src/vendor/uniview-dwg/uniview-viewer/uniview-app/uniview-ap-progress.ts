/**
 * Configuration options for {@link UvApProgress}.
 */
export interface UvApProgressOptions {
  /**
 ...
 * @example
 * ```ts
 * const progress = new UvApProgress({ message: "Loading data…" });
 * progress.show();
 *
 * setTimeout(() => {
...
export class UvApProgress {
   ...
   */
  public readonly options: Required<UvApProgressOptions>

  /**
   * Creates a new fullscreen infinite progress indicator.
   *
   * @param options - Optional {@link UvApProgressOptions} controlling appearance & behavior
   */
  constructor(options: UvApProgressOptions = {}) {
    this.options = {
    ...
    if (!UvApProgress.stylesInjected) {
      this.injectStyles()
    }
   ...
   *
   * @returns The current {@link UvApProgress} instance (for chaining)
   */
  public show(): this {
  ...
  private injectStyles(): void {
    if (document.getElementById(UvApProgress.styleId)) {
      UvApProgress.stylesInjected = true
      return
    }
