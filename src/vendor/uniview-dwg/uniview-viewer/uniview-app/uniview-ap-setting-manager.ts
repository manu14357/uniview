 ...
 * @example
 * ```typescript
 * const fontMapping: UvApFontMapping = {
 *   'AutoCAD Font': 'Arial',
 *   'SimSun': 'Microsoft YaHei'
 * };
 * ```
 */
export type UvApFontMapping = Record<string, string>
...
export interface UvApSettings {
  ...
  /** Whether the axes gizmo (X/Y orientation indicator) is visible */
  isShowAxesGizmo: boolean
  /** Whether performance statistics are displayed */
  isShowStats: boolean
  /** Whether the built-in progress overlay is displayed during file loading */
  isShowProgress: boolean
  /** Font mapping configuration for text rendering */
  fontMapping: UvApFontMapping
  /** Object snap modes */
  osnapModes: number
}

/** Default values for all application settings */
const DEFAULT_VALUES: UvApSettings = {
  isDebug: false,
  ...
  isShowToolbar: false,
  isShowAxesGizmo: false,
  isShowStats: false,
  isShowProgress: false,
  fontMapping: {},
  osnapModes: uvdbOsnapModesToMask([
    UvDbOsnapMode.EndPoint,
    UvDbOsnapMode.MidPoint,
    UvDbOsnapMode.Center
  ])
}
 ...
 *
 * @template T - The settings type, defaults to UvApSettings
 */
export interface UvApSettingManagerEventArgs<
  T extends UvApSettings = UvApSettings
> {
 ...
 *
 * @template T - The settings interface type, defaults to UvApSettings
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export class UvApSettingManager<T extends UvApSettings = UvApSettings> {
  /** Singleton instance */
  private static _instance?: UvApSettingManager

  /** Events fired when settings are modified */
  public readonly events = {
    /** Fired when any setting is modified */
    modified: new UvCmEventManager<UvApSettingManagerEventArgs<T>>()
  }
  ...
  static get instance() {
    if (!this._instance) {
      this._instance = new UvApSettingManager()
    }
  ...
  /**
   * Gets whether the axes gizmo (X/Y orientation indicator) is visible.
   *
   * @returns True if axes gizmo should be shown
   */
  get isShowAxesGizmo() {
    return this.get('isShowAxesGizmo')
  }

  /**
   * Sets whether the axes gizmo is visible.
   *
   * @param value - True to show the axes gizmo
   */
  set isShowAxesGizmo(value: boolean) {
    this.set('isShowAxesGizmo', value)
  }
  ...
  /**
   * Gets whether the built-in progress overlay is displayed.
   *
   * @returns True if progress overlay should be shown
   */
  get isShowProgress() {
    return this.get('isShowProgress')
  }

  /**
   * Sets whether the built-in progress overlay is displayed.
   *
   * @param value - True to show the progress overlay
   */
  set isShowProgress(value: boolean) {
    this.set('isShowProgress', value)
  }
   ...
   */
  set fontMapping(value: UvApFontMapping) {
  ...
  setFontMapping(originalFont: string, mappedFont: string) {
    const mapping = this.get('fontMapping') as UvApFontMapping
