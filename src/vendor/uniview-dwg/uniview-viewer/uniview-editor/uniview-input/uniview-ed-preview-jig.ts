import { UvDbEntity } from '@uniview/data-model'

import { UvEdBaseView } from '../uniview-view/uniview-ed-base-view'

/**
 * @class UvEdPreviewJig<T>
 * @template T
 *
 * @description
 * A generic, TypeScript-based version of AutoCAD’s UvEdJig system for
 ...
 *
 * ## Method Mapping vs AutoCAD UvEdJig
 * | AutoCAD UvEdJig Method | Purpose | This Class |
 * |-------------------------|---------|------------|
 ...
 *
 * ## Example Use Cases
 * - Point acquisition (`T = UvGePoint3d`)
 * - Distance jig (`T = number`)
 * - Angle jig (`T = number`)
 *
 * Example:
 * ```ts
 * const jig = new UvEdPreviewJig<UvGePoint3d>(view)
 * jig.setEntity(circleEntity)
 * await editor.getPoint("Specify center:", { jig })
 * ```
 */
export abstract class UvEdPreviewJig<T> {
   ...
   */
  abstract get entity(): UvDbEntity | null
