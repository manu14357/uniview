import { UvEdKeyword } from './uniview-ed-keyword'

/**
 * A collection of `UvEdKeyword` objects, mirroring `Autodesk.AutoCAD.EditorInput.KeywordCollection`.
 * Represents the set of valid keywords for a prompt.
 */
export class UvEdKeywordCollection {
  private _keywords: UvEdKeyword[] = []
  private _defaultKeyword?: UvEdKeyword
   ...
   *
   * ```ts
   * new UvEdKeywordCollection("Width Height Depth")
   * ```
   *
   * ### Abbreviation rules
   ...
   * @returns The newly created `UvEdKeyword`.
   */
  add(displayName: string): UvEdKeyword
  /**
   ...
   * @returns The newly created `UvEdKeyword`.
   */
  add(globalName: string, localName: string): UvEdKeyword
  /**
  ...
  get default(): UvEdKeyword | undefined {
    return this._defaultKeyword
  }
  set default(kw: UvEdKeyword | undefined) {
  ...
  /** Returns an iterator over the `UvEdKeyword` objects in this collection. */
  [Symbol.iterator](): Iterator<UvEdKeyword> {
    let index = 0
    const arr = this._keywords
    return {
      next(): IteratorResult<UvEdKeyword> {
  ...
  findByDisplayName(displayName: string): UvEdKeyword | undefined {
  ...
  findByGlobalName(globalName: string): UvEdKeyword | undefined {
    return this._keywords.find(
  ...
  findByName(name: string): UvEdKeyword | undefined {
