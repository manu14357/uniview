import { UvGePoint3d } from '@uniview/data-model'

import { UvEdPreviewJig } from '../uniview-ed-preview-jig'
import { UvEdKeywordCollection } from './uniview-ed-keyword-collection'
 ...
 */
export class UvEdPromptOptions<T = number | string | UvGePoint3d> {
  private _jig?: UvEdPreviewJig<T>
  private _message: string
  private _appendKeywordsToMessage: boolean
  private _isReadOnly: boolean
  private _keywords: UvEdKeywordCollection

  /**
   * Constructs a new `UvEdPromptOptions` with a given prompt message.
  ...
  constructor(message: string) {
    ...
    this._keywords = new UvEdKeywordCollection()
  }
  ...
  set jig(jig: UvEdPreviewJig<T> | undefined) {
    if (!this._isReadOnly) {
      this._jig = jig
    }
  }
  ...
  /**
   * Gets whether this `UvEdPromptOptions` is read-only.
   ...
   */
  get keywords(): UvEdKeywordCollection {
