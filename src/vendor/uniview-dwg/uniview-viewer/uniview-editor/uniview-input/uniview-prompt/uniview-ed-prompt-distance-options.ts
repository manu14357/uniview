...
import { UvEdPromptNumericalOptions } from './uniview-ed-prompt-numerical-options'
 ...
 */
export class UvEdPromptDistanceOptions extends UvEdPromptNumericalOptions {
  ...
  /**
   * Constructs a new `UvEdPromptDistanceOptions` with a prompt message.
  ...
  set basePoint(point: UvGePoint3d | undefined) {
    if (!this.isReadOnly) {
      if (point == null) {
