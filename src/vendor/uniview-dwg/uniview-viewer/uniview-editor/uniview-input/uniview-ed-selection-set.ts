 ...
 */
export interface UvEdSelectionEventArgs {
 ...
 * @example
 * ```typescript
 * const selectionSet = new UvEdSelectionSet();
 *
...
export class UvEdSelectionSet {
   ...
   * const selectionSet = new UvEdSelectionSet(['entity1', 'entity2']);
   * ```
   */
  constructor(ids: UvDbObjectId[] = []) {
    this._ids = new Set<UvDbObjectId>(ids)
  }
   ...
   * ```
   */
  add(value: UvDbObjectId | UvDbObjectId[]) {
   ...
   * ```
   */
  delete(value: UvDbObjectId | UvDbObjectId[]) {
    if (Array.isArray(value)) {
