...
import { UvEdPromptKeywordOptions } from '../uniview-prompt'
import { UvEdKeywordSession } from '../uniview-session'
...
export class UvEdCommandLine {
  ...
  private activeSession?: UvEdKeywordSession
  ...
  constructor(container: HTMLElement = document.body) {
    ...
    UvApI18n.events.localeChanged.addEventListener(() => this.refreshLocale())
  }
  ...
  async getKeywords(options: UvEdPromptKeywordOptions): Promise<string> {
    this.activeSession = new UvEdKeywordSession(this, options)
  ...
  /**
   * Localize a text key using UvApI18n.t().
   *
   ...
   * @returns localized string from UvApI18n or the provided defaultText
   */
  private localize(key: string, defaultText?: string): string {
    return UvApI18n.t(key, { fallback: defaultText })
  }
  ...
  executeCommand(cmdLine: string) {
    ...
    UvApDocManager.instance.sendStringToExecute(cmdLine)
    this.clearInput()
  }
  ...
  private bindEvents() {
    ...
    this.closeBtn.addEventListener('click', e => {
      e.stopPropagation()
      this.visible = false
      UvApSettingManager.instance.isShowCommandLine = false
    })
  ...
  private handleInputChange() {
    ...
    const matches = UvApDocManager.instance.searchCommandsByPrefix(text)
    if (matches.length) {
      this.autoCompleteIndex = -1
      ...
      matches.forEach((item, idx) => {
        ...
        const description = UvApI18n.cmdDescription(
          item.commandGroup,
  ...
  renderKeywordPrompt(
    options: UvEdPromptKeywordOptions,
  ...
  private resolveCommand(cmdLine: string) {
    ...
    return UvApDocManager.instance.lookupLocalCmd(cmdStr)
  }
