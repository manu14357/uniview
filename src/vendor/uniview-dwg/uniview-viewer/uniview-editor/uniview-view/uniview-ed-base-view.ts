...
import { UvEdCorsorType, UvEdSelectionSet } from '../uniview-input'
import { UvEditor } from '../uniview-input/uniview-editor'
import { UvEdHoverController } from './uniview-ed-hover-controller'
import { UvEdSpatialQueryResultItemEx } from './uniview-ed-spatial-query-result'

/**
 * Interface to define arguments of mouse event events.
 */
export interface UvEdMouseEventArgs {
  /**
 ...
 */
export interface UvEdViewResizedEventArgs {
  /**
 ...
 */
export interface UvEdViewHoverEventArgs {
  /**
 ...
 * view.mode = UvEdViewMode.SELECTION;
 *
 * // Set to pan mode for view navigation
 * view.mode = UvEdViewMode.PAN;
 * ```
 */
export enum UvEdViewMode {
  /**
 ...
 */
export interface UvEdMissedData {
 ...
 */
export type UvEdCalculateSizeCallback = () => { width: number; height: number }
 ...
 * @example
 * ```typescript
 * class MyView extends UvEdBaseView {
 ...
 * ```
 */
export abstract class UvEdBaseView {
  /** Current viewport width in pixels */
  private _width: number
  /** Current viewport height in pixels */
  private _height: number
  /** Optional callback to calculate canvas size on resize */
  private _calculateSizeCallback?: UvEdCalculateSizeCallback
  /** Bounding box of all entities in the view */
  private _bbox: UvGeBox3d
  /** Current mouse position in world coordinates */
  private _curPos: UvGePoint2d
  /**
   ...
   */
  private _curMousePos: UvGePoint2d
  /** Set of currently selected entities */
  private _selectionSet: UvEdSelectionSet
  /** Input manager for handling user interactions */
  private _editor: UvEditor
   ...
   *
   * Extracted into a separate class to keep `UvEdBaseView` focused on
   * view lifecycle and input routing rather than interaction timing logic.
   */
  private _hoverController: UvEdHoverController
  ...
  public readonly events = {
    ...
    mouseMove: new UvCmEventManager<UvEdMouseEventArgs>(),
    /** Fired when the view is resized */
    viewResize: new UvCmEventManager<UvEdViewResizedEventArgs>(),
    /** Fired when the view camera is changed */
    viewChanged: new UvCmEventManager<void>(),
    /** Fired when mouse hovers over an entity */
    hover: new UvCmEventManager<UvEdViewHoverEventArgs>(),
    /** Fired when mouse stops hovering over an entity */
    unhover: new UvCmEventManager<UvEdViewHoverEventArgs>()
  }
  ...
  constructor(canvas: HTMLCanvasElement, container: HTMLElement) {
    ...
    this._bbox = new UvGeBox3d()
    this._width = rect.width
    this._height = rect.height
    this._curPos = new UvGePoint2d()
    this._curMousePos = new UvGePoint2d()
    this._selectionSet = new UvEdSelectionSet()
    this._editor = new UvEditor(this)
    this._canvas.addEventListener('mousemove', event => this.onMouseMove(event))
    this._canvas.addEventListener('mousedown', event => {
      if (event.button === 1) {
        // Middle mouse button (button === 1)
        this._editor.setCursor(UvEdCorsorType.Grab)
      }
    })
    ...
    this._hoverController = new UvEdHoverController(
      this,
      this.events.hover,
      this.events.unhover
    )
   ...
   */
  abstract get missedData(): UvEdMissedData
   ...
   */
  abstract get mode(): UvEdViewMode
   ...
   */
  abstract set mode(value: UvEdViewMode)
   ...
   */
  abstract get center(): UvGePoint2d
   ...
   */
  abstract set center(value: UvGePoint2d)
   ...
   * ```
   */
  abstract screenToWorld(point: UvGePoint2dLike): UvGePoint2d
   ...
   * @example
   * ```typescript
   * const worldPoint = new UvGePoint2d(10, 20); // CAD coordinates
   * const screenPoint = view.worldToScreen(worldPoint);
   * console.log('Screen position:', screenPoint.x, screenPoint.y);
   * ```
   */
  abstract worldToScreen(point: UvGePoint2dLike): UvGePoint2d
   ...
   */
  abstract zoomTo(box: UvGeBox2d, margin: number): void
   ...
   */
  abstract flyTo(point: UvGePoint2dLike, scale?: number): void
   ...
   */
  abstract search(box: UvGeBox2d | UvGeBox3d): UvEdSpatialQueryResultItemEx[]
   ...
   */
  abstract selectByBox(box: UvGeBox2d): void
   ...
   */
  abstract select(point?: UvGePoint2dLike): void
   ...
   */
  abstract addLayer(layer: UvDbLayerTableRecord): void
   ...
   */
  abstract addTransientEntity(entity: UvDbEntity | UvDbEntity[]): void
   ...
   */
  abstract removeTransientEntity(objectId: UvDbObjectId): void
   ...
   */
  abstract addEntity(entity: UvDbEntity | UvDbEntity[]): void
   ...
   */
  abstract removeEntity(entity: UvDbEntity | UvDbEntity[]): void

  /**
   * Update the specified entity or entities
   * @param entity Input the entity or entities to update
   */
  abstract updateEntity(entity: UvDbEntity | UvDbEntity[]): void
   ...
   */
  abstract addLayout(layout: UvDbLayout): void

  /**
   * Select the specified entities
   */
  abstract highlight(ids: UvDbObjectId[]): void
  /**
   * Unhighlight the specified entities
   */
  abstract unhighlight(ids: UvDbObjectId[]): void

  /**
   * Called when hovering the specified entity
   */
  abstract onHover(id: UvDbObjectId): void

  /**
   * Called when unhovering the specified entity
   */
  abstract onUnhover(id: UvDbObjectId): void
   ...
   */
  setCalculateSizeCallback(value: UvEdCalculateSizeCallback) {
   ...
   */
  viewportToCanvas(point: UvGePoint2dLike): UvGePoint2d {
    const rect = this._canvas.getBoundingClientRect()
    return new UvGePoint2d(point.x - rect.left, point.y - rect.top)
  }

  /**
   * Converts canvas-local coordinates into viewport/client-space coordinates.
   */
  canvasToViewport(point: UvGePoint2dLike): UvGePoint2d {
    const rect = this._canvas.getBoundingClientRect()
    return new UvGePoint2d(point.x + rect.left, point.y + rect.top)
  }
   ...
   */
  canvasToContainer(point: UvGePoint2dLike): UvGePoint2d {
    const canvasRect = this._canvas.getBoundingClientRect()
    const containerRect = this._container.getBoundingClientRect()
    return new UvGePoint2d(
   ...
   */
  viewportToContainer(point: UvGePoint2dLike): UvGePoint2d {
    const containerRect = this._container.getBoundingClientRect()
    return new UvGePoint2d(
