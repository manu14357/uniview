import { UvDbObjectId, UvGePoint2d } from '@uniview/data-model'
import { UvCmEventManager } from '@uniview/data-model'

import { UvEdViewHoverEventArgs } from './uniview-ed-base-view'

/**
 * Interface implemented by a view that hosts hover behavior.
 *
 * `UvEdHoverController` is deliberately UI-framework-agnostic and relies
 ...
 */
export interface UvEdHoverHost {
  /**
   ...
   */
  pick(point: { x: number; y: number }): { id: UvDbObjectId }[]
   ...
   */
  onUnhover(id: UvDbObjectId): void
 ...
 * or UI frameworks. All such responsibilities are delegated to `UvEdHoverHost`.
 */
export class UvEdHoverController {
  /**
   ...
   */
  private hoveredId: UvDbObjectId | null = null
   ...
   */
  constructor(
    private readonly host: UvEdHoverHost,
    private readonly hoverEvent: UvCmEventManager<UvEdViewHoverEventArgs>,
    private readonly unhoverEvent: UvCmEventManager<UvEdViewHoverEventArgs>,
   ...
   */
  private setHoveredId(newId: UvDbObjectId | null) {
    if (this.hoveredId) {
   ...
   * @param id - Object id of the hovered entity
   */
  private startPauseTimer(id: UvDbObjectId) {
    this.clearPauseTimer()
