 ...
 * 2. `UvEditor.commandWillStart` event is fired
 * 3. `execute()` method is called with current context
 * 4. `UvEditor.commandEnded` event is fired
 *
 * @example
 * ```typescript
 * class MyDrawCommand extends UvEdCommand {
 *   constructor() {
 *     super();
 ...
 * ```
 */
export abstract class UvEdCommand<TUserData extends object = {}> {
  ...
  private _mode: UvEdOpenMode = UvEdOpenMode.Read
   ...
   * }
   *
   * class MyCommand extends UvEdCommand<MyCommandData> {}
   *
   ...
   */
  protected onCommandWillStart(_context: UvApContext): void {
   ...
   * @param _context - The current application context
   */
  protected onCommandEnded(_context: UvApContext): void {
    // Intentionally empty — override in subclasses if needed
  }
   ...
   * ```
   */
  async trigger(context: UvApContext) {
    try {
