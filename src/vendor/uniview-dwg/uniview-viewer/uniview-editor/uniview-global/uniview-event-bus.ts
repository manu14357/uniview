import { UvDbProgressdEventArgs, UvGePoint3dLike } from '@uniview/data-model'
 ...
 */
export type UvEdMessageType = 'success' | 'warning' | 'info' | 'error'

export interface UvEdFontNotLoadedInfo {
  /** Font name */
  fontName: string
...
export type MeasurementRecord =
  | { type: 'distance'; p1: UvGePoint3dLike; p2: UvGePoint3dLike; dist: number }
  | { type: 'area'; points: UvGePoint3dLike[]; area: number }
  | {
      type: 'arc'
      geom: { cx: number; cy: number; r: number }
      start: UvGePoint3dLike
      end: UvGePoint3dLike
      arcLen: number
      mid: UvGePoint3dLike
    }
 ...
 * @example
 * ```typescript
 * import { eventBus } from './uniview-event-bus';
 *
 ...
 *   percentage: 50,
 *   stage: 'CONVERSION'
 *   subStage: UvDbConversionStage.Parsing,
 *   stageStatus: UvDbConversionStageStatus.InProgress
 * });
 *
 ...
 * ```
 */
export const eventBus: Emitter<UvEdEvents> = mitt<UvEdEvents>()
