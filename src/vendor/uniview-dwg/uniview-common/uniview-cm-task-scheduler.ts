 ...
 * @example
 * ```typescript
 * class LoadFileTask extends UvCmTask<string, ArrayBuffer> {
 *   constructor() {
 *     super('LoadFile')
 *   }
 *
 ...
 * ```
 */
export class UvCmTask<TIn, TOut> {
  /**
 ...
 * @param {UvCmTask<unknown, unknown>} task - The task that was just completed.
 */
type UvCmProgressCallback = (
  progress: number,
  task: UvCmTask<unknown, unknown>
) => void
 ...
 */
export type UvCmCompleteCallback<T> = (finalResult: T) => void
 ...
 *
 * @param {UvCmTaskError} error - Detailed information about the task error.
 * @returns {boolean} Whether to interrupt the task execution flow.
 */
type UvCmErrorCallback = (error: UvCmTaskError) => boolean
 ...
 * const scheduler = new UvCmTaskScheduler<string, ParsedData>()
 *
 * // Add tasks
 ...
 * ```
 */
export class UvCmTaskScheduler<TInitial, TFinal = TInitial> {
  private tasks: UvCmTask<unknown, unknown>[] = []
  private onProgress: UvCmProgressCallback = () => {}
  private onComplete: UvCmCompleteCallback<TFinal> = () => {}
  private onError: UvCmErrorCallback = () => false
   ...
   */
  addTask<TIn, TOut>(task: UvCmTask<TIn, TOut>): void {
    this.tasks.push(task as UvCmTask<unknown, unknown>)
  }

  /**
   * Sets a callback to receive progress updates.
   */
  setProgressCallback(callback: UvCmProgressCallback): void {
   ...
   */
  setCompleteCallback(callback: UvCmCompleteCallback<TFinal>): void {
   ...
   */
  setErrorCallback(callback: UvCmErrorCallback): void {
