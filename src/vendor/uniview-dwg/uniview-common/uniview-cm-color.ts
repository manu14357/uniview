...
import { UvCmColorUtil } from './uniview-cm-color-util'
...
export class UvCmColor {
  ...
  /**
   * Constructs a new UvCmColor.
   * @param method Initial color method (defaults to `ByColor`)
   * @param value Internal packed value
   */
  constructor(
    method: UvCmColorMethod = UvCmColorMethod.ByLayer,
    value?: number
  ) {
    this._colorMethod = method
    if (this._colorMethod == UvCmColorMethod.ByColor && value == null) {
      this._value = 0xffffff
    } else if (this._colorMethod == UvCmColorMethod.ByACI) {
      if (value == null) {
        this._value = 8
      } else if (value === 0) {
        this._colorMethod = UvCmColorMethod.ByBlock
      } else if (value === 256) {
   ...
   * - For `ByACI`, converts index to RGB via `UvCmColorUtil`
   * - For `ByLayer` or `ByBlock`, returns `_value` directly
   */
  get RGB(): number | undefined {
    switch (this._colorMethod) {
      case UvCmColorMethod.ByColor:
      case UvCmColorMethod.ByBlock:
      case UvCmColorMethod.ByLayer:
        return this._value
      case UvCmColorMethod.ByACI:
        return this._value
          ? UvCmColorUtil.getColorByIndex(this._value)
          : this._value
      default:
        return undefined
    }
  ...
  setRGBFromCss(cssString: string) {
    ...
    // Named color
    const namedColor = UvCmColorUtil.getColorByName(cssString)
  ...
  get colorIndex(): number | undefined {
    if (this._colorMethod === UvCmColorMethod.ByACI) return this._value
    else if (this._colorMethod === UvCmColorMethod.ByLayer) return 256
    else if (this._colorMethod === UvCmColorMethod.ByBlock) return 0
   ...
   * Gets the color name.
   *
   * For `ByColor` or `ByACI`, resolves the name via `UvCmColorUtil`.
  ...
  get colorName(): string | undefined {
    switch (this._colorMethod) {
      ...
      case UvCmColorMethod.ByBlock:
        return 'ByBlock'
      case UvCmColorMethod.ByColor:
        return this._value ? UvCmColorUtil.getNameByColor(this._value) : ''
      case UvCmColorMethod.ByACI:
        return this._value ? UvCmColorUtil.getNameByIndex(this._value) : ''
      default:
        return undefined
    }
  }
   ...
   *
   * Resolves the name to an RGB value via `UvCmColorUtil`.
   *
   * @param name Color name
   */
  set colorName(name: string | undefined) {
    if (!name) return
    const color = UvCmColorUtil.getColorByName(name)
    if (color !== undefined) {
      this._value = color
      this._colorMethod = UvCmColorMethod.ByColor
    } else {
   ...
   *
   * @returns A new UvCmColor instance with the same method and value
   */
  clone(): UvCmColor {
    const c = new UvCmColor()
  ...
  /**
   * Copies color values from another UvCmColor instance.
   *
   * @param other The source color
   * @returns The current instance
   */
  copy(other: UvCmColor) {
  ...
  /**
   * Creates one UvCmColor from one string
   */
  static fromString(name: string): UvCmColor | undefined {
    ...
    // -------------------------------------------------
    if (/^bylayer$/i.test(n)) {
      return new UvCmColor(UvCmColorMethod.ByLayer)
    }

    if (/^byblock$/i.test(n)) {
      return new UvCmColor(UvCmColorMethod.ByBlock)
    }
    ...
    if (rgbPrefixedMatch) {
      ...
      const color = new UvCmColor(UvCmColorMethod.ByColor)
      color.setRGB(r, g, b)
      return color
    }
    ...
    if (/^\d+$/.test(n)) {
      ...
      return new UvCmColor(UvCmColorMethod.ByACI, index)
    }
    ...
    if (/^book\$/i.test(n)) {
      ...
      const colorVal = UvCmColorUtil.getColorByName(bookName)
      if (colorVal != null) {
        return new UvCmColor(UvCmColorMethod.ByColor, colorVal)
      }
    ...
    // 6. Named colors
    // -------------------------------------------------
    const colorVal = UvCmColorUtil.getColorByName(n)
    if (colorVal != null) {
      return new UvCmColor(UvCmColorMethod.ByColor, colorVal)
    }
