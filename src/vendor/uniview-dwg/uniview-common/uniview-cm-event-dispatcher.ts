 ...
 *
 * @module UvCmEventDispatcher
 * @version 1.0.0
 */

/**
 * The minimal basic Event that can be dispatched by a {@link UvCmEventDispatcher<>}.
 *
 * @template TEventType - The string literal type of the event.
 */
export interface UvCmBaseEvent<TEventType extends string = string> {
  /** The type identifier for this event. */
  readonly type: TEventType
}

/**
 * The minimal expected contract of a fired Event that was dispatched by a {@link UvCmEventDispatcher<>}.
 *
 * @template TEventType - The string literal type of the event.
 * @template TTarget - The type of the object that dispatched the event.
 */
export interface UvCmEvent<
 ...
 * @template TTarget - The type of the object that dispatched the event.
 */
export type UvCmEventListener<
  TEventData,
  TEventType extends string,
  TTarget
> = (event: TEventData & UvCmEvent<TEventType, TTarget>) => void

/**
 * Interface for a type-safe event dispatcher.
 *
 * Consumers should depend on this interface rather than the concrete
 * UvCmEventDispatcher class (Dependency Inversion Principle).
 *
 * @template TEventMap - A record type mapping event names to their data payloads.
 */
// eslint-disable-next-line
export interface IUvCmEventDispatcher<TEventMap extends {} = {}> {
  addEventListener<T extends Extract<keyof TEventMap, string>>(
    type: T,
    listener: UvCmEventListener<TEventMap[T], T, this>
  ): void
  ...
  dispatchEvent<T extends Extract<keyof TEventMap, string>>(
    event: UvCmBaseEvent<T> & TEventMap[T]
  ): void
}
 ...
 *
 * // Create dispatcher
 * const dispatcher = new UvCmEventDispatcher<MyEvents>()
 *
 * // Add listeners
 ...
 */
// eslint-disable-next-line
export class UvCmEventDispatcher<TEventMap extends {} = {}> implements IUvCmEventDispatcher<TEventMap> {
  /**
  ...
  /**
   * Creates {@link UvCmEventDispatcher} object.
   */
  constructor() {
    this._listeners = {}
  }
  ...
  addEventListener<T extends string>(
    type: T,
    listener: UvCmEventListener<object, T, this>
  ): void {
