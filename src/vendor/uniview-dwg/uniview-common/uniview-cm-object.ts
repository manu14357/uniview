...
import { UvCmEventManager } from './uniview-cm-event-manager'
import { clone, defaults, has, isEmpty, isEqual } from './uniview-cm-lodash-utils'

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export type UvCmAttributes = Record<string, any>

/**
 * Options to set attributes of one `UvCmObject` instance
 */
export interface UvCmObjectOptions {
  silent?: boolean | undefined
  unset?: boolean | undefined
}

export type UvCmStringKey<T> = keyof T & string

/**
 * Interface to define arguments of changed event of one `UvCmObject` instance
 */
export interface UvCmObjectChangedEventArgs<T extends UvCmAttributes> {
  object: UvCmObject<T>
  options?: UvCmObjectOptions
}

/**
 * Interface to define arguments of attribute changed event of one `UvCmObject` instance
 */
export interface UvCmObjectAttributeChangedEventArgs<T extends UvCmAttributes>
  extends UvCmObjectChangedEventArgs<T> {
  attrName?: string
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  attrValue?: any
}
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export class UvCmObject<T extends UvCmAttributes = any> {
  attributes: Partial<T>
  changed: Partial<T>

  public readonly events = {
    attrChanged: new UvCmEventManager<UvCmObjectAttributeChangedEventArgs<T>>(),
    modelChanged: new UvCmEventManager<UvCmObjectChangedEventArgs<T>>()
  }
   ...
   * ```
   */
  get<A extends UvCmStringKey<T>>(key: A): T[A] | undefined {
   ...
   * @param {T[A] | UvCmObjectOptions} [val] - The value to set or options when key is an object.
   * @param {UvCmObjectOptions} [options] - Options for the set operation.
   ...
   * ```
   */
  set<A extends UvCmStringKey<T>>(
    key: A,
    val?: T[A],
    options?: UvCmObjectOptions
  ): this
  set(key: Partial<T>, options?: UvCmObjectOptions): this
  set<A extends UvCmStringKey<T>>(
    key: A | Partial<T>,
    val?: T[A] | UvCmObjectOptions,
    options?: UvCmObjectOptions
  ): this {
    ...
    if (typeof key === 'object') {
      ...
      options = val as UvCmObjectOptions
    } else {
      attrs = {} as Partial<T>
      attrs[key] = val as T[A]
    }

    options || (options = {} as UvCmObjectOptions)
  ...
  has(key: UvCmStringKey<T>): boolean {
   ...
   */
  hasChanged(key?: UvCmStringKey<T>) {
   ...
   */
  previous<A extends UvCmStringKey<T>>(key: A): T[A] | null | undefined {
