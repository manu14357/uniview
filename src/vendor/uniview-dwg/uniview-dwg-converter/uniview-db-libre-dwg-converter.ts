...
/**
 * Database converter for DWG files based on [libredwg-web](https://github.com/nicholasgasior/libredwg).
 */
export class UvDbLibreDwgConverter extends UvDbDatabaseConverter<DwgDatabase> {
  ...
  protected processLineTypes(model: DwgDatabase, db: UvDbDatabase) {
    const lineTypes = model.tables.LTYPE.entries
    lineTypes.forEach(item => {
      const lineType: UvGiBaseLineStyle = {
        name: item.name,
      ...
      const record = new UvDbLinetypeTableRecord(lineType)
  ...
  protected processTextStyles(model: DwgDatabase, db: UvDbDatabase) {
  ...
  protected processDimStyles(model: DwgDatabase, db: UvDbDatabase) {
    ...
    dimStyles.forEach(item => {
      const attrs: UvDbDimStyleTableRecordAttrs = {
        ...
        dimazin: item.DIMAZIN as unknown as UvDbDimZeroSuppressionAngular.None,
        dimalt: item.DIMALT,
        ...
        dimsd1: item.DIMSD1,
        dimsd2: item.DIMSD2,
        dimtolj: item.DIMTOLJ as unknown as UvDbDimVerticalJustification.Bottom,
  ...
  protected processLayers(model: DwgDatabase, db: UvDbDatabase) {
  ...
  protected processViewports(model: DwgDatabase, db: UvDbDatabase) {
    ...
    viewports.forEach(item => {
      ...
      if (item.renderMode) {
        record.gsView.renderMode = item.renderMode as unknown as UvGiRenderMode
      }
      if (item.viewMode) {
  ...
  protected processHeader(model: DwgDatabase, db: UvDbDatabase) {
  ...
  protected processObjects(model: DwgDatabase, db: UvDbDatabase) {
    this.processLayouts(model, db)
    this.processImageDefs(model, db)
  }

  private processLayouts(model: DwgDatabase, db: UvDbDatabase) {
    const layoutDict = db.objects.layout
    const layouts = model.objects.LAYOUT
    layouts.forEach(layout => {
      const dbLayout = new UvDbLayout()
